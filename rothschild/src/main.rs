...
use velkar_consensus_core::{
    ...
    constants::{SOMPI_PER_VELKAR, TX_VERSION},
    network::NetworkType,
    sign::sign,
    subnets::SUBNETWORK_ID_NATIVE,
    tx::{MutableTransaction, Transaction, TransactionInput, TransactionOutpoint, TransactionOutput, UtxoEntry},
};
use velkar_core::{info, velkard_env::version, time::unix_now, warn};
use velkar_grpc_client::{ClientPool, GrpcClient};
use velkar_notify::subscription::context::SubscriptionContext;
use velkar_rpc_core::{RpcUtxoEntry, api::rpc::RpcApi, notify::mode::NotificationMode};
...
const DEFAULT_SEND_AMOUNT: u64 = 10 * SOMPI_PER_VELKAR;
...
#[tokio::main]
async fn main() {
    velkar_core::log::init_logger(None, "");
    let args = Args::parse();
    ...
    let schnorr_key = if let Some(private_key_hex) = args.private_key {
        ...
        let velkar_addr = Address::new(address_prefix, ADDRESS_VERSION, &pk.x_only_public_key().0.serialize());
        info!(
            "Generated private key {} and address {}. Send some funds to this address and rerun rothschild with `--private-key {}`",
            sk.display_secret(),
            String::from(&velkar_addr),
            sk.display_secret()
        );
        return;
    };

    let velkar_addr = Address::new(address_prefix, ADDRESS_VERSION, &schnorr_key.x_only_public_key().0.serialize());

    let velkar_to_addr = args.addr.as_ref().map_or_else(|| velkar_addr.clone(), |addr_str| Address::try_from(addr_str.clone()).unwrap());
    ...
    if args.addr.is_some() {
        log_message.push_str(&format!("\n\tto address: {}", String::from(&velkar_to_addr)));
    }
...
\tMedian time: {}, \n\tDAA score: {}, \n\tPruning point: {}, \n\tTips: {}, \n\t{} virtual parents: ...{}, \n\tCoinbase maturity: {}",
    ...
    let mut utxos = refresh_utxos(&rpc_client, velkar_addr.clone(), &mut pending, coinbase_maturity).await;
    ...
    loop {
        ...
        if !has_funds || now - last_refresh > 60_000 {
            ...
            utxos = refresh_utxos(&rpc_client, velkar_addr.clone(), &mut pending, coinbase_maturity).await;
...
async fn refresh_utxos(
    ...
    populate_pending_outpoints_from_mempool(rpc_client, velkar_addr.clone(), pending).await;
    fetch_spendable_utxos(rpc_client, velkar_addr, coinbase_maturity, pending).await
}
...
async fn populate_pending_outpoints_from_mempool(
    ...
    let entries = rpc_client.get_mempool_entries_by_addresses(vec![velkar_addr], true, false).await.unwrap();
    let now = Instant::now();
...
async fn fetch_spendable_utxos(
    ...
    let resp = rpc_client.get_utxos_by_addresses(vec![velkar_addr]).await.unwrap();
    ...
    let txs = selected_utxos_groups
        ...
        .map(|utxo_option| {
            if let Some((selected_utxos, selected_amount)) = utxo_option {
                let tx = generate_tx(schnorr_key, &selected_utxos, selected_amount, num_outs, &velkar_addr, tx_config.payload_size);
...
fn generate_tx(
    ...
    let script_public_key = pay_to_address_script(velkar_addr);
    let inputs = utxos
        .iter()

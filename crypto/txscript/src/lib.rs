...
use velkar_consensus_core::hashing::sighash::{
...
use velkar_consensus_core::tx::{PopulatedTransaction, ScriptPublicKey, TransactionInput, UtxoEntry, VerifiableTransaction};
use velkar_txscript_errors::TxScriptError;
...
impl<'a, T: VerifiableTransaction, Reused: SigHashReusedValues> TxScriptEngine<'a, T, Reused> {
    ...
    pub fn execute_opcode(&mut self, opcode: DynOpcodeImplementation<T, Reused>) -> Result<(), TxScriptError> {
        ...
        // Different from velkard: Illegal and disabled opcode are checked on execute instead
        // Note that this includes OP_RESERVED which counts as a push operation.
...
mod bitcoind_tests {
    ...
    use velkar_consensus_core::constants::MAX_TX_IN_SEQUENCE_NUM;
    use velkar_consensus_core::hashing::sighash::SigHashReusedValuesUnsync;
    use velkar_consensus_core::tx::{

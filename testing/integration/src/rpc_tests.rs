...
use velkar_addresses::{Address, Prefix, Version};
use velkar_consensus::params::SIMNET_GENESIS;
use velkar_consensus_core::{constants::MAX_SOMPI, header::Header, subnets::SubnetworkId, tx::Transaction};
use velkar_core::{assert_match, info};
use velkar_grpc_core::ops::VelkardPayloadOps;
...
use velkar_rpc_core::{Notification, api::rpc::RpcApi, model::*};
use velkar_utils::{fd_budget, networking::ContextualNetAddress};
use velkard_lib::args::Args;
...
/// `cargo test --release --package velkar-testing-integration --lib -- rpc_tests::sanity_test`
#[tokio::test]
async fn sanity_test() {
    velkar_core::log::try_init_logger("info");
    // As we log the panic, we want to set it up after the logger
    velkar_core::panic::configure_panic();
    ...
    for op in VelkardPayloadOps::iter() {
        let network_id = daemon.network;
        let task: JoinHandle<()> = match op {
            VelkardPayloadOps::SubmitBlock => {
            ...
            VelkardPayloadOps::GetBlockTemplate => {
                tst!(op, "see SubmitBlock")
            }

            VelkardPayloadOps::GetCurrentBlockColor => {
                tst!(op, "see SubmitBlock")
            }

            VelkardPayloadOps::GetCurrentNetwork => {
                let rpc_client = client.clone();
                tst!(op, {
            ...
            VelkardPayloadOps::GetInfo => {
                let rpc_client = client.clone();
                tst!(op, {
                    let response = rpc_client.get_info_call(None, GetInfoRequest {}).await.unwrap();
                    assert_eq!(response.server_version, velkar_core::velkard_env::version().to_string());
            ...
            VelkardPayloadOps::GetMempoolEntries => {
                let rpc_client = client.clone();
                tst!(op, {
                    let response = rpc_client
            ...
            VelkardPayloadOps::GetConnectedPeerInfo => {
                let rpc_client = client.clone();
                tst!(op, {
            ...
            VelkardPayloadOps::Unban => {
                tst!(op, "see Ban")
            }

            VelkardPayloadOps::SubmitTransaction => {
                let rpc_client = client.clone();
                tst!(op, {
            ...
            VelkardPayloadOps::SubmitTransactionReplacement => {
                let rpc_client = client.clone();
                tst!(op, {
                    // Build an erroneous transaction...
                    let transaction = Transaction::new(0, vec![], vec![], 0, SubnetworkId::default(), 0, vec![]);
            ...
            VelkardPayloadOps::GetSubnetwork => {
                let rpc_client = client.clone();
                tst!(op, {
                    let result =
            ...
            VelkardPayloadOps::GetVirtualChainFromBlock => {
                tst!(op, "see SubmitBlock")
            }

            VelkardPayloadOps::GetBlockCount => {
                tst!(op, "see SubmitBlock")
            }

            VelkardPayloadOps::GetBlockDagInfo => {
                let rpc_client = client.clone();
                tst!(op, {
            ...
            VelkardPayloadOps::ResolveFinalityConflict => {
                let rpc_client = client.clone();
                tst!(op, {
                    let response_result = rpc_client
            ...
            VelkardPayloadOps::GetUtxosByAddresses => {
                let rpc_client = client.clone();
                tst!(op, {
            ...
            VelkardPayloadOps::GetBalanceByAddress => {
                let rpc_client = client.clone();
                tst!(op, {
                    let response = rpc_client
            ...
            VelkardPayloadOps::GetBalancesByAddresses => {
                let rpc_client = client.clone();
                tst!(op, {
            ...
            VelkardPayloadOps::GetSinkBlueScore => {
                let rpc_client = client.clone();
                tst!(op, {
                    let response = rpc_client.get_sink_blue_score_call(None, GetSinkBlueScoreRequest {}).await.unwrap();
            ...
            VelkardPayloadOps::EstimateNetworkHashesPerSecond => {
                let rpc_client = client.clone();
                tst!(op, {
                    let response_result = rpc_client
            ...
            VelkardPayloadOps::GetMempoolEntriesByAddresses => {
                let rpc_client = client.clone();
                tst!(op, {
                    let addresses = vec![Address::new(Prefix::Simnet, Version::PubKey, &[0u8; 32])];
                    let response = rpc_client
            ...
            VelkardPayloadOps::GetCoinSupply => {
                let rpc_client = client.clone();
                tst!(op, {
            ...
            VelkardPayloadOps::GetConnections => {
                let rpc_client = client.clone();
                tst!(op, {
            ...
            VelkardPayloadOps::GetSystemInfo => {
                let rpc_client = client.clone();
                tst!(op, {
            ...
            VelkardPayloadOps::GetServerInfo => {
                let rpc_client = client.clone();
                tst!(op, {
            ...
            VelkardPayloadOps::GetSyncStatus => {
                let rpc_client = client.clone();
                tst!(op, {
            ...
            VelkardPayloadOps::GetDaaScoreTimestampEstimate => {
                let rpc_client = client.clone();
                tst!(op, {
            ...
            VelkardPayloadOps::GetFeeEstimate => {
                let rpc_client = client.clone();
                tst!(op, {
            ...
            VelkardPayloadOps::GetFeeEstimateExperimental => {
                let rpc_client = client.clone();
                tst!(op, {
            ...
            VelkardPayloadOps::GetUtxoReturnAddress => {
                let rpc_client = client.clone();
                tst!(op, {
                    ...
                    assert!(results.is_err_and(|err| {
                        match err {
                            velkar_rpc_core::RpcError::General(msg) => {
            ...
            VelkardPayloadOps::GetVirtualChainFromBlockV2 => {
                let rpc_client = client.clone();
                tst!(op, {
                    let response = rpc_client
            ...
            VelkardPayloadOps::NotifyBlockAdded => {
                let rpc_client = client.clone();
                let id = listener_id;
                tst!(op, {
            ...
            VelkardPayloadOps::NotifyNewBlockTemplate => {
                let rpc_client = client.clone();
                let id = listener_id;
                tst!(op, {
            ...
            VelkardPayloadOps::NotifyFinalityConflict => {
                let rpc_client = client.clone();
                let id = listener_id;
                tst!(op, {
            ...
            VelkardPayloadOps::NotifyUtxosChanged => {
                let rpc_client = client.clone();
                let id = listener_id;
                tst!(op, {
            ...
            VelkardPayloadOps::NotifySinkBlueScoreChanged => {
                let rpc_client = client.clone();
                let id = listener_id;
                tst!(op, {
            ...
            VelkardPayloadOps::NotifyPruningPointUtxoSetOverride => {
                let rpc_client = client.clone();
                let id = listener_id;
                tst!(op, {
            ...
            VelkardPayloadOps::NotifyVirtualDaaScoreChanged => {
                let rpc_client = client.clone();
                let id = listener_id;
                tst!(op, {
            ...
            VelkardPayloadOps::NotifyVirtualChainChanged => {
                let rpc_client = client.clone();
                let id = listener_id;
                tst!(op, {
                    rpc_client
            ...
            VelkardPayloadOps::StopNotifyingUtxosChanged => {
                let rpc_client = client.clone();
                let id = listener_id;
                tst!(op, {
            ...
            VelkardPayloadOps::StopNotifyingPruningPointUtxoSetOverride => {
                let rpc_client = client.clone();
                let id = listener_id;
                tst!(op, {

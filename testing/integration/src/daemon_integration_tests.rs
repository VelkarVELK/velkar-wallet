...
use velkar_consensus::params::SIMNET_PARAMS;
use velkar_consensus_core::header::Header;
use velkar_consensusmanager::ConsensusManager;
use velkar_core::{task::runtime::AsyncRuntime, trace};
use velkar_grpc_client::GrpcClient;
use velkar_notify::scope::{BlockAddedScope, UtxosChangedScope, VirtualDaaScoreChangedScope};
use velkar_rpc_core::{Notification, RpcTransactionId, api::rpc::RpcApi};
...
async fn daemon_sanity_test() {
    ...
    // let total_fd_limit =  velkar_utils::fd_budget::get_limit() / 2 - 128;
    let total_fd_limit = 10;
    let mut velkard1 = Daemon::new_random(total_fd_limit);
    let rpc_client1 = velkard1.start().await;
    assert!(rpc_client1.handle_message_id() && rpc_client1.handle_stop_notify(), "the client failed to collect server features");

    let mut velkard2 = Daemon::new_random(total_fd_limit);
    let rpc_client2 = velkard2.start().await;
...
async fn daemon_mining_test() {
    ...
    let mut velkard1 = Daemon::new_random_with_args(args.clone(), total_fd_limit);
    let mut velkard2 = Daemon::new_random_with_args(args, total_fd_limit);
    let rpc_client1 = velkard1.start().await;
    let rpc_client2 = velkard2.start().await;

    rpc_client2.add_peer(format!("127.0.0.1:{}", velkard1.p2p_port).try_into().unwrap(), true).await.unwrap();
    ...
    for i in 0..10 {
        let template = rpc_client1
            .get_block_template(Address::new(velkard1.network.into(), velkar_addresses::Version::PubKey, &[0; 32]), vec![])
            .await
            .unwrap();
    ...
    let vc = rpc_client2
        .get_virtual_chain_from_block(
            velkar_consensus::params::SIMNET_GENESIS.hash, //
            true,
            None,
        )
        .await
...
/// `cargo test --release --package velkar-testing-integration --lib -- daemon_integration_tests::daemon_utxos_propagation_test`
#[tokio::test(flavor = "multi_thread", worker_threads = 1)]
async fn daemon_utxos_propagation_test() {
    #[cfg(feature = "heap")]
    let _profiler = dhat::Profiler::builder().file_name("velkar-testing-integration-heap.json").build();

    velkar_core::log::try_init_logger(
        "INFO,velkar_testing_integration=trace,velkar_notify=debug,velkar_rpc_core=debug,velkar_grpc_client=debug",
    );
    ...
    let miner_address =
        Address::new(velkard1.network.into(), velkar_addresses::Version::PubKey, &miner_pk.x_only_public_key().0.serialize());
    ...
    let user_address =
        Address::new(velkard1.network.into(), velkar_addresses::Version::PubKey, &user_pk.x_only_public_key().0.serialize());

    // Some dummy non-monitored address
    let blank_address = Address::new(velkard1.network.into(), velkar_addresses::Version::PubKey, &[0; 32]);
    ...
    // Check that acceptance data contains the expected coinbase tx ids
    let vc = rpc_client2.get_virtual_chain_from_block(velkar_consensus::params::SIMNET_GENESIS.hash, true, None).await.unwrap();
    assert_eq!(vc.removed_chain_block_hashes.len(), 0);
    ...
    let mut clients = vec![ListeningClient::connect(&velkard2).await, ListeningClient::connect(&velkard1).await];
...
async fn daemon_cleaning_test() {
    init_allocator_with_default_settings();
    velkar_core::log::try_init_logger("info,velkar_grpc_core=trace,velkar_grpc_server=trace,velkar_grpc_client=trace,velkar_core=trace");
    ...
    {
        let total_fd_limit = 10;
        let mut velkard1 = Daemon::new_random_with_args(args, total_fd_limit);
        let dyn_consensus_manager = velkard1.core.find(ConsensusManager::IDENT).unwrap();
        let dyn_async_runtime = velkard1.core.find(AsyncRuntime::IDENT).unwrap();
        ...
        core = Arc::downgrade(&velkard1.core);

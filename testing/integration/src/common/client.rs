...
use velkar_notify::{events::EventType, scope::Scope, subscription::Command};
...
impl ListeningClient {
    pub async fn connect(velkard: &Daemon) -> Self {
        let client = velkard.new_multi_listener_client().await;

...
use velkar_consensus::config::{Config, ConfigBuilder};
use velkar_consensus::consensus::factory::Factory as ConsensusFactory;
use velkar_consensus::consensus::test_consensus::{TestConsensus, TestConsensusFactory};
use velkar_consensus::model::stores::block_transactions::{
    BlockTransactionsStore, BlockTransactionsStoreReader, DbBlockTransactionsStore,
};
use velkar_consensus::model::stores::ghostdag::{GhostdagStoreReader, KType as GhostdagKType};
use velkar_consensus::model::stores::headers::HeaderStoreReader;
use velkar_consensus::model::stores::reachability::DbReachabilityStore;
use velkar_consensus::model::stores::relations::DbRelationsStore;
use velkar_consensus::model::stores::selected_chain::SelectedChainStoreReader;
use velkar_consensus::params::{DEVNET_PARAMS, ForkActivation, MAINNET_PARAMS, OverrideParams};
use velkar_consensus::pipeline::ProcessingCounters;
use velkar_consensus::pipeline::monitor::ConsensusMonitor;
use velkar_consensus::processes::reachability::tests::{DagBlock, DagBuilder, StoreValidationExtensions};
use velkar_consensus::processes::window::{WindowManager, WindowType};
use velkar_consensus_core::api::args::TransactionValidationArgs;
use velkar_consensus_core::api::{BlockValidationFutures, ConsensusApi};
use velkar_consensus_core::block::Block;
use velkar_consensus_core::blockhash::{self, new_unique};
use velkar_consensus_core::blockstatus::BlockStatus;
use velkar_consensus_core::coinbase::MinerData;
use velkar_consensus_core::constants::{BLOCK_VERSION, SOMPI_PER_VELKAR, TRANSIENT_BYTE_TO_MASS_FACTOR};
use velkar_consensus_core::errors::block::{BlockProcessResult, RuleError};
use velkar_consensus_core::hashing;
use velkar_consensus_core::header::Header;
use velkar_consensus_core::mining_rules::MiningRules;
use velkar_consensus_core::subnets::SubnetworkId;
use velkar_consensus_core::tx::{
    MutableTransaction, ScriptPublicKey, Transaction, TransactionInput, TransactionOutpoint, TransactionOutput, UtxoEntry,
};
use velkar_consensus_notify::root::ConsensusNotificationRoot;
use velkar_consensus_notify::service::NotifyService;
use velkar_consensusmanager::ConsensusManager;
use velkar_core::task::tick::TickService;
use velkar_core::time::unix_now;
use velkar_database::utils::get_velkar_tempdir;
use velkar_hashes::Hash;
use velkar_rpc_core::RpcHeader;
use velkar_utils::arc::ArcExtensions;
...
use velkar_consensus_core::hashing::sighash::calc_schnorr_signature_hash;
use velkar_consensus_core::muhash::MuHashExtensions;
use velkar_core::core::Core;
use velkar_core::signals::Shutdown;
use velkar_core::task::runtime::AsyncRuntime;
use velkar_core::{assert_match, info};
use velkar_database::create_temp_db;
use velkar_database::prelude::{CachePolicy, ConnBuilder};
use velkar_index_processor::service::IndexService;
use velkar_math::Uint256;
use velkar_muhash::MuHash;
use velkar_notify::subscription::context::SubscriptionContext;
use velkar_txscript::caches::TxScriptCacheCounters;
use velkar_txscript::opcodes::codes::OpTrue;
use velkar_txscript::script_builder::ScriptBuilderResult;
use velkar_utxoindex::UtxoIndex;
use velkar_utxoindex::api::{UtxoIndexApi, UtxoIndexProxy};
...
async fn json_test(file_path: &str, concurrency: bool) {
    velkar_core::log::try_init_logger("info");
...
async fn staging_consensus_test() {
    ...
    let db_tempdir = get_velkar_tempdir();
    let db_path = db_tempdir.path().to_owned();
    let consensus_db_dir = db_path.join("consensus");
    let meta_db_dir = db_path.join("meta");

    let meta_db = velkar_database::prelude::ConnBuilder::default().with_db_path(meta_db_dir).with_files_limit(5).build().unwrap();
    ...
    let consensus_factory = Arc::new(ConsensusFactory::new(
        ...
        velkar_database::prelude::RocksDbPreset::Default,
        None,
        None,
    ));
...
#[tokio::test]
async fn kip10_test() {
    use velkar_consensus_core::subnets::SUBNETWORK_ID_NATIVE;
    use velkar_txscript::opcodes::codes::{Op0, OpTxInputSpk};
    use velkar_txscript::pay_to_script_hash_script;
    use velkar_txscript::script_builder::ScriptBuilder;
    ...
    let initial_utxo_collection = [(
        ...
        UtxoEntry { amount: SOMPI_PER_VELKAR, script_public_key: spk.clone(), block_daa_score: 0, is_coinbase: false },
    )];
...
async fn runtime_sig_op_counting_test() {
    ...
    let script_pub_key = velkar_txscript::pay_to_script_hash_script(&redeem_script);

    // Set up initial UTXO with P2SH script
    let initial_utxo_collection = [(
        TransactionOutpoint::new(1.into(), 0),
        UtxoEntry { amount: SOMPI_PER_VELKAR, script_public_key: script_pub_key.clone(), block_daa_score: 0, is_coinbase: false },
    )];
...
async fn sighash_type_commitment_test() {
    use velkar_consensus_core::hashing::sighash::SigHashReusedValuesUnsync;
    use velkar_consensus_core::hashing::sighash_type::{
        SIG_HASH_ALL, SIG_HASH_ANY_ONE_CAN_PAY, SIG_HASH_NONE, SIG_HASH_SINGLE, SigHashType,
    };
    use velkar_consensus_core::subnets::SUBNETWORK_ID_NATIVE;
    use velkar_txscript::opcodes::codes::*;
    use velkar_txscript::pay_to_script_hash_script;
    use velkar_txscript::script_builder::ScriptBuilder;
    ...
    for i in 0..6 {
        initial_utxo_collection.push((
            ...
            UtxoEntry { amount: SOMPI_PER_VELKAR / 10, script_public_key: p2sh_script.clone(), block_daa_score: 0, is_coinbase: false },
        ));
    }
    for i in 0..3 {
        initial_utxo_collection.push((
            TransactionOutpoint::new((i + 7).into(), 0),
            UtxoEntry { amount: SOMPI_PER_VELKAR / 20, script_public_key: op_true_spk.clone(), block_daa_score: 0, is_coinbase: false },
        ));
    }

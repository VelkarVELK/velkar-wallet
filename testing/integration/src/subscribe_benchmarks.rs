...
use velkar_consensus::params::Params;
use velkar_consensus_core::network::{NetworkId, NetworkType};
use velkar_core::{info, task::tick::TickService, trace};
use velkar_math::Uint256;
use velkar_notify::scope::VirtualDaaScoreChangedScope;
use velkar_rpc_core::api::rpc::RpcApi;
...
fn create_client_addresses(index: usize, network_id: &NetworkId) -> Vec<Address> {
    ...
    (min_address..max_address)
        .map(|x| Address::new((*network_id).into(), velkar_addresses::Version::PubKey, &Uint256::from_u64(x as u64).to_le_bytes()))
        .collect_vec()
}

/// `cargo test --package velkar-testing-integration --lib --features devnet-prealloc -- subscribe_benchmarks::utxos_changed_subscriptions_sanity_check --exact --nocapture --ignored`
#[tokio::test]
#[ignore = "bmk"]
async fn utxos_changed_subscriptions_sanity_check() {
    init_allocator_with_default_settings();
    velkar_core::log::try_init_logger(
        "INFO, velkar_core::time=debug, velkar_rpc_core=debug, velkar_grpc_client=debug, velkar_notify=info, velkar_notify::address::tracker=debug, velkar_notify::listener=debug, velkar_notify::subscription::single=debug, velkar_mining::monitor=debug, velkar_testing_integration::subscribe_benchmarks=trace",
    );
    ...
    // Fold-up
    //
    velkar_core::info!("Signal the daemon to shutdown");
    client.shutdown().await.unwrap();
    velkar_core::warn!("Disconnect the main client");
    client.disconnect().await.unwrap();
    drop(client);

    velkar_core::warn!("Waiting for the daemon to exit...");
    daemon_process.wait().await.expect("failed to wait for the daemon process");
}

/// `cargo test --package velkar-testing-integration --lib --features devnet-prealloc -- subscribe_benchmarks::bench_utxos_changed_subscriptions_daemon --exact --nocapture --ignored -- --rpc=16610 --p2p=16611 --private-key=a2760251adb5b6e8d4514d23397f1631893e168c33f92ff8a7a24f397d355d62 --max-tracked-addresses=1000000 --utxoindex`
///
...
async fn bench_utxos_changed_subscriptions_daemon() {
    init_allocator_with_default_settings();
    velkar_core::log::try_init_logger(
        "INFO, velkar_core::core=trace, velkar_core::time=debug, velkar_rpc_core=debug, velkar_grpc_client=debug, velkar_notify=info, velkar_notify::address::tracker=debug, velkar_notify::listener=debug, velkar_notify::subscription::single=debug, velkar_mining::monitor=debug, velkar_testing_integration::subscribe_benchmarks=trace",
    );
    // As we log the panic, we want to set it up after the logger
    velkar_core::panic::configure_panic();
...
async fn utxos_changed_subscriptions_client(address_cycle_seconds: u64, address_max_cycles: usize) {
    ...
    let prealloc_address =
        Address::new(NetworkType::Simnet.into(), velkar_addresses::Version::PubKey, &prealloc_pk.x_only_public_key().0.serialize());
...
/// `cargo test --package velkar-testing-integration --lib --features devnet-prealloc -- subscribe_benchmarks::bench_utxos_changed_subscriptions_footprint_a --exact --nocapture --ignored`
#[tokio::test]
#[ignore = "bmk"]
...
/// `cargo test --package velkar-testing-integration --lib --features devnet-prealloc -- subscribe_benchmarks::bench_utxos_changed_subscriptions_footprint_b --exact --nocapture --ignored`
#[tokio::test]
#[ignore = "bmk"]
...
/// `cargo test --package velkar-testing-integration --lib --features devnet-prealloc -- subscribe_benchmarks::bench_utxos_changed_subscriptions_footprint_c --exact --nocapture --ignored`
#[tokio::test]
#[ignore = "bmk"]
...
/// `cargo test --package velkar-testing-integration --lib --features devnet-prealloc -- subscribe_benchmarks::bench_utxos_changed_subscriptions_footprint_d --exact --nocapture --ignored`
#[tokio::test]
#[ignore = "bmk"]
...
/// `cargo test --package velkar-testing-integration --lib --features devnet-prealloc -- subscribe_benchmarks::bench_utxos_changed_subscriptions_footprint_e --exact --nocapture --ignored`
#[tokio::test]
#[ignore = "bmk"]

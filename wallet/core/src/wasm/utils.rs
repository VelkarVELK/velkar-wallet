...
use js_sys::BigInt;
use velkar_consensus_core::network::{NetworkIdT, NetworkType, NetworkTypeT};
...
extern "C" {
    ...
    #[derive(Clone, Debug)]
    pub type ISompiToVelkar;
}

/// Convert a Velkar string to Sompi represented by bigint.
/// This function provides correct precision handling and
/// can be used to parse user input.
/// @category Wallet SDK
#[wasm_bindgen(js_name = "velkarToSompi")]
pub fn velkar_to_sompi(velkar: String) -> Option<BigInt> {
    crate::utils::try_velkar_str_to_sompi(velkar).ok().flatten().map(Into::into)
}

///
/// Convert Sompi to a string representation of the amount in Velkar.
///
/// @category Wallet SDK
///
#[wasm_bindgen(js_name = "sompiToVelkarString")]
pub fn sompi_to_velkar_string(sompi: ISompiToVelkar) -> Result<String> {
    let sompi = sompi.try_as_u64()?;
    Ok(crate::utils::sompi_to_velkar_string(sompi))
}

///
/// Format a Sompi amount to a string representation of the amount in Velkar with a suffix
/// based on the network type (e.g. `VELK` for mainnet, `TVELK` for testnet,
/// `SVELK` for simnet, `DVELK` for devnet).
///
/// @category Wallet SDK
///
#[wasm_bindgen(js_name = "sompiToVelkarStringWithSuffix")]
pub fn sompi_to_velkar_string_with_suffix(sompi: ISompiToVelkar, network: &NetworkTypeT) -> Result<String> {
    let sompi = sompi.try_as_u64()?;
    let network_type = NetworkType::try_from(network)?;
    Ok(crate::utils::sompi_to_velkar_string_with_suffix(sompi, &network_type))
}

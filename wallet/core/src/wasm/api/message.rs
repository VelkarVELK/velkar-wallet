 ...
 * Wallet API for interfacing with Rusty Velkar Wallet implementation.
 */
"#;
...
declare! {
     ...
     * with the default filename `velkar`.
     * 
     * @category Wallet API
     */
...
try_from! (args: IAccountsCreateRequest, AccountsCreateRequest, {
    ...
    let account_create_args = match kind.as_str() {
        ...
        _ => {
            return Err(Error::custom("only BIP32/velkar-keypair-standard accounts are currently supported"));
        }
    };

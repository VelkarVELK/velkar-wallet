...
use velkar_wrpc_client::VelkarRpcClient;
...
impl UtxoProcessor {
    ...
    pub fn rpc_client(&self) -> Option<Arc<VelkarRpcClient>> {
        self.rpc_api().clone().downcast_arc::<VelkarRpcClient>().ok()
    }
    ...
    pub async fn init_state_from_server(&self) -> Result<bool> {
        ...
        log_trace!("Connected to velkard: '{server_version}' on '{server_network_id}';  SYNC: {is_synced}  DAA: {virtual_daa_score}");

...
//! (Partial Signed Velkar Transaction Bundles).
//!
...
use futures::stream;
use velkar_bip32::{DerivationPath, KeyFingerprint, PrivateKey};
use velkar_consensus_client::UtxoEntry as ClientUTXO;
use velkar_consensus_core::hashing::sighash::{SigHashReusedValuesUnsync, calc_schnorr_signature_hash};
use velkar_consensus_core::tx::VerifiableTransaction;
use velkar_consensus_core::tx::{TransactionInput, UtxoEntry};
use velkar_txscript::extract_script_pub_key_address;
use velkar_txscript::opcodes::codes::OpData65;
use velkar_txscript::script_builder::ScriptBuilder;
use velkar_wallet_core::tx::{Generator, GeneratorSettings, PaymentDestination, PendingTransaction};
pub use velkar_wallet_pskt::bundle::Bundle;
use velkar_wallet_pskt::bundle::{script_sig_to_address, unlock_utxo_outputs_as_batch_transaction_pskb};
use velkar_wallet_pskt::prelude::KeySource;
use velkar_wallet_pskt::prelude::lock_script_sig_templating_bytes;
use velkar_wallet_pskt::prelude::{Finalizer, Inner, SignInputOk, Signature, Signer};
pub use velkar_wallet_pskt::pskt::{Creator, PSKT};
...
pub fn pskt_to_pending_transaction(
    ...
    let output: &Vec<velkar_consensus_core::tx::TransactionOutput> = &signed_tx.outputs;
    if output.is_empty() {
    ...
    let pending_tx = PendingTransaction::try_new(
        ...
        0,
        0,
        velkar_wallet_core::tx::DataKind::Final,
    )?;

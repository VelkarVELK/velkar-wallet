...
pub fn default_storage_folder() -> &'static str {
    ...
    unsafe {
        DEFAULT_STORAGE_FOLDER.get_or_insert("~/.velkar".to_string()).as_str()
    }
}
...
pub fn default_wallet_file() -> &'static str {
    ...
    #[allow(static_mut_refs)]
    unsafe {
        DEFAULT_WALLET_FILE.get_or_insert("velkar".to_string()).as_str()
    }
}
...
pub fn default_settings_file() -> &'static str {
    ...
    #[allow(static_mut_refs)]
    unsafe {
        DEFAULT_SETTINGS_FILE.get_or_insert("velkar".to_string()).as_str()
    }
}
...
/// the storage folder will default to `~/.velkar`

...
use crate::{tx::PaymentOutputs, utils::velkar_to_sompi};
use velkar_addresses::Address;
use velkar_consensus_core::config::params::Params;
use velkar_consensus_core::mass::UtxoCell;
use velkar_consensus_core::network::{NetworkId, NetworkType};
use velkar_consensus_core::tx::Transaction;
use rand::prelude::*;
...
impl From<Velkar> for Sompi {
    fn from(velkar: Velkar) -> Self {
        Sompi(velkar_to_sompi(velkar.0))
    }
}
    ...
    F: FnOnce(NetworkType) -> Address,
{
    ...
    let utxo_entries: Vec<UtxoEntryReference> = values.into_iter().map(velkar_to_sompi).map(UtxoEntryReference::simulated).collect();
...
pub(crate) fn change_address(network_type: NetworkType) -> Address {
    match network_type {
        NetworkType::Mainnet => Address::try_from("velkar:qpauqsvk7yf9unexwmxsnmg547mhyga37csh0kj53q6xxgl24ydxjsgzthw5j").unwrap(),
        NetworkType::Testnet => Address::try_from("velkartest:qqz22l98sf8jun72rwh5rqe2tm8lhwtdxdmynrz4ypwak427qed5juktjt7ju").unwrap(),
...
pub(crate) fn output_address(network_type: NetworkType) -> Address {
    match network_type {
        NetworkType::Mainnet => Address::try_from("velkar:qrd9efkvg3pg34sgp6ztwyv3r569qlc43wa5w8nfs302532dzj47knu04aftm").unwrap(),
        NetworkType::Testnet => Address::try_from("velkartest:qqrewmx4gpuekvk8grenkvj2hp7xt0c35rxgq383f6gy223c4ud5s58ptm6er").unwrap(),
        _ => unreachable!("network type not supported"),
    }
}
...
fn test_generator_sweep_two_utxos() -> Result<()> {
    make_generator(test_network_id(), &[10.0, 10.0], &[], None, Fees::None, change_address, PaymentDestination::Change)
        ...
        .fetch(&Expected {
            is_final: true,
            input_count: 2,
            aggregate_input_value: Velkar(20.0),
            output_count: 1,
...
fn test_generator_compound_200k_10kas_transactions() -> Result<()> {
    generator(
        ...
        None,
        Fees::sender(Velkar(5.0)),
        [(output_address, Velkar(190_000.0))].as_slice(),
    )
    .unwrap()
    .harness()
    .validate()
...
fn test_generator_compound_100k_random_transactions() -> Result<()> {
    ...
    let outputs = [(output_address, Velkar(total - 10.0))];
    generator(test_network_id(), &inputs, &[], None, Fees::sender(Velkar(5.0)), outputs.as_slice())
        .unwrap()
        .harness()
        .validate()
        .finalize();
...
fn test_generator_random_outputs() -> Result<()> {
    ...
    let outputs: Vec<_> = outputs.into_iter().map(|v| (output_address, Velkar(v))).collect();

    generator(test_network_id(), &[total + 100.0], &[], None, Fees::sender(Velkar(5.0)), outputs.as_slice())
        .unwrap()
        .harness()
        .validate()
        .finalize();
...
fn test_generator_dust_1_1() -> Result<()> {
    generator(
        ...
        None,
        Fees::sender(Velkar(5.0)),
        [(output_address, Velkar(1.0)), (output_address, Velkar(1.0))].as_slice(),
    )
    .unwrap()
    .harness()
    .fetch(&Expected {
        is_final: true,
        input_count: 4,
        aggregate_input_value: Velkar(40.0),
        output_count: 3,
        priority_fees: FeesExpected::sender(Velkar(5.0)),
    })
    .finalize();
...
fn test_generator_inputs_2_outputs_2_fees_exclude() -> Result<()> {
    generator(
        ...
        None,
        Fees::sender(Velkar(5.0)),
        [(output_address, Velkar(10.0)), (output_address, Velkar(1.0))].as_slice(),
    )
    .unwrap()
    .harness()
    .fetch(&Expected {
...
fn test_generator_inputs_100_outputs_1_fees_exclude_success() -> Result<()> {
    // generator(test_network_id(), &[10.0; 100], &[], Fees::sender(Velkar(5.0)), [(output_address, Velkar(990.0))].as_slice())
    generator(test_network_id(), &[10.0; 100], &[], None, Fees::sender(Velkar(0.0)), [(output_address, Velkar(990.0))].as_slice())
        .unwrap()
        .harness()
        .fetch(&Expected {
            is_final: false,
            input_count: 88,
            aggregate_input_value: Velkar(880.0),
            output_count: 1,
            priority_fees: FeesExpected::None,
        })
        .fetch(&Expected {
            is_final: false,
            input_count: 12,
            aggregate_input_value: Velkar(120.0),
            output_count: 1,
            priority_fees: FeesExpected::None,
        })
        .fetch(&Expected {
            ...
            output_count: 2,
            // priority_fees: FeesExpected::sender(Velkar(5.0)),
            priority_fees: FeesExpected::sender(Velkar(0.0)),
        })
        .finalize();
...
fn test_generator_inputs_100_outputs_1_fees_include_success() -> Result<()> {
    generator(
        ...
        &[],
        None,
        Fees::receiver(Velkar(5.0)),
        // [(output_address, Velkar(100.0))].as_slice(),
        [(output_address, Velkar(100.0))].as_slice(),
    )
    .unwrap()
    .harness()
    .fetch(&Expected {
        is_final: false,
        input_count: 88,
        aggregate_input_value: Velkar(88.0),
        output_count: 1,
        priority_fees: FeesExpected::None,
    })
    .fetch(&Expected {
        is_final: false,
        input_count: 12,
        aggregate_input_value: Velkar(12.0),
        output_count: 1,
        priority_fees: FeesExpected::None,
    })
    .fetch(&Expected {
        ...
        output_count: 1,
        priority_fees: FeesExpected::receiver(Velkar(5.0)),
    })
    .finalize();
...
fn test_generator_inputs_100_outputs_1_fees_exclude_insufficient_funds() -> Result<()> {
    generator(test_network_id(), &[10.0; 100], &[], None, Fees::sender(Velkar(5.0)), [(output_address, Velkar(1000.0))].as_slice())
        .unwrap()
        .harness()
        .fetch(&Expected {
            is_final: false,
...
fn test_generator_inputs_1k_outputs_2_fees_exclude() -> Result<()> {
    generator(test_network_id(), &[10.0; 1_000], &[], None, Fees::sender(Velkar(5.0)), [(output_address, Velkar(9_000.0))].as_slice())
        .unwrap()
        .harness()
        .drain(
            10,
        ...
        .fetch(&Expected {
            is_final: false,
            input_count: 21,
            aggregate_input_value: Velkar(210.0),
            output_count: 1,
            priority_fees: FeesExpected::None,
        })
        .fetch(&Expected {
...
fn test_generator_inputs_32k_outputs_2_fees_exclude() -> Result<()> {
    ...
    generator(
        ...
        &[],
        None,
        Fees::sender(Velkar(10_000.0)),
        [(output_address, Velkar(f * 32_747.0 - 10_001.0))].as_slice(),
    )
    .unwrap()
    .harness()
    .accumulate(379)
...
fn test_generator_fan_out_1() -> Result<()> {
    use velkar_consensus_core::mass::calc_storage_mass;
    ...
    //     1.00000000,
    //     87.23579967,
    // ], &[], None, Fees::sender(Velkar(1.0)), [
    //     (output_address, Velkar(0.20000000)),
    //     (output_address, Velkar(0.25000000)),
    //     (output_address, Velkar(0.21000000)),
    // ].as_slice())
    //     .unwrap()
    //     .harness()
    //     // .accumulate(1)
    ...
    //         is_final: true,
    //         input_count: 2,
    //         aggregate_input_value: Velkar(1.00000000 + 87.23579967),
    //         output_count: 4,
    //         priority_fees: FeesExpected::receiver(Velkar(1.0)),
    //         // priority_fees: FeesExpected::None,
    //     })
    //     .finalize();

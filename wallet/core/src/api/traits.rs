//!
//! API trait for interfacing with the Velkar wallet subsystem.
//!
...
pub trait WalletApi: Send + Sync + AnySync {
    ...
    /// If the `filename` is `None`, the wallet opens the default wallet named `velkar`.
    ///
    ...
    /// applications such as KDX and velkarnet.io web wallet using a deprecated derivation path
    ...
    /// Wrapper around [`accounts_pskb_create()`](Self::accounts_pskb_create_call)
    async fn accounts_pskb_create(self: Arc<Self>, request: AccountsPskbCreateRequest) -> Result<AccountsPskbCreateResponse> {
        self.accounts_pskb_create_call(request).await
    }

    /// Create an unsigned PSKB.
    async fn accounts_pskb_create_call(
        self: Arc<Self>,
        request: AccountsPskbCreateRequest,
    ) -> Result<AccountsPskbCreateResponse>;

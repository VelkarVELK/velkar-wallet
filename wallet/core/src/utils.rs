//!
//! Velkar value formatting and parsing utilities.
//!
...
pub fn try_velkar_str_to_sompi<S: Into<String>>(s: S) -> Result<Option<u64>> {
    let s: String = s.into();
    let amount = s.trim();
    if amount.is_empty() {
        return Ok(None);
...
pub fn try_velkar_str_to_sompi_i64<S: Into<String>>(s: S) -> Result<Option<i64>> {
    let s: String = s.into();
    let amount = s.trim();
    if amount.is_empty() {
        return Ok(None);
    ...
    let amount = amount.parse::<f64>()? * SOMPI_PER_VELKAR as f64;
...
#[inline]
pub fn sompi_to_velkar_string_with_trailing_zeroes_and_suffix(sompi: u64, network_type: &NetworkType) -> String {
    let vlk = sompi_to_velkar_string_with_trailing_zeroes(sompi);
    let suffix = velkar_suffix(network_type);
    format!("{vlk} {suffix}")
}
...
fn str_to_sompi(amount: &str) -> Result<u64> {
    let Some(dot_idx) = amount.find('.') else {
        return Ok(amount.parse::<u64>()? * SOMPI_PER_VELKAR);
    };
    let integer = amount[..dot_idx].parse::<u64>()? * SOMPI_PER_VELKAR;

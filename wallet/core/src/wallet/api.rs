...
use crate::tx::Fees;
use velkar_rpc_core::RpcFeeEstimate;
...
impl WalletApi for super::Wallet {
    ...
    async fn accounts_pskb_create_call(self: Arc<Self>, request: AccountsPskbCreateRequest) -> Result<AccountsPskbCreateResponse> {
        let AccountsPskbCreateRequest { account_id, wallet_secret, payment_secret, destination, fee_rate, priority_fee_sompi, payload } =
            request;
        ...
        let abortable = Abortable::new();
        let pskb = account
            .pskb_from_send_generator(destination, fee_rate, priority_fee_sompi, payload, wallet_secret, payment_secret, &abortable)
            .await?;

        Ok(AccountsPskbCreateResponse { pskb: pskb.serialize()? })
    }

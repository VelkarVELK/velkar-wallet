...
const fs = require('fs');
const velkar = require('../../../../nodejs/velkar-dev');
const {
...
(async()=>{
    ...
    async function log_transactions(accountId){
        ...
        transactionsResult.transactions.forEach((tx)=>{
            ...
            list.push({
                Id: tx.id,
                Type: tx.data.type,
                Value: sompiToVelkarString(value(tx)||0)
            });
    ...
    try {
                ...
                case "balance":
                    ...
                    Object.keys(balance).map(id=>{
                        ...
                        list.push({
                            ...
                            Mature: sompiToVelkarString(b.mature),
                            Pending: sompiToVelkarString(b.pending),
                            Outgoing: sompiToVelkarString(b.outgoing),
        ...
        // Send velkar to address
        let sendResult = await wallet.accountsSend({
            walletSecret,
            // @ts-ignore
            accountId: firstAccount.accountId,
            priorityFeeSompi: velkarToSompi("0.001"),
            destination:[{
                // @ts-ignore
                address: firstAccount.changeAddress,
                amount: velkarToSompi("1.567")
            }]
        });
        ...
        // Transfer velkar between accounts
        let transferResult = await wallet.accountsTransfer({
            ...
            transferAmountSompi: velkarToSompi("2.456"),
        });

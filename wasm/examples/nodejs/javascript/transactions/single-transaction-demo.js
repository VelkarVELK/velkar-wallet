...
(async () => {
    ...
    // An address such as velkar:qr0lr4ml9fn3chekrqmjdkergxl93l4wrk3dankcgvjq776s9wn9jkdskewva
    const address = keypair.toAddress(networkId);
    console.info(`Full velkar address: ${address}`);
    ...
    try {
        ...
        if (utxos.length === 0) {
            console.info('Send some velkar to', address, 'before proceeding with the demo');
            return;
        }

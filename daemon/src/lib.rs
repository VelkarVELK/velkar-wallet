...
pub use velkard::{Velkard, VelkardConfig, VelkardCtl};
...
pub static LOCATIONS: &[&str] = &[
    ...
    "../target/release",
    "../target/debug",
    "../../velkar-cpu-miner/target/debug",
    "../../velkar-cpu-miner/target/release",
    "bin/windows-x64",
    "bin/linux-ia32",
    "bin/linux-x64",
    "bin/linux-arm64",
...
#[derive(Default)]
pub struct Daemons {
    pub velkard: Option<Arc<dyn VelkardCtl + Send + Sync + 'static>>,
    // pub velkard_automute : Arc<
    pub cpu_miner: Option<Arc<dyn CpuMinerCtl + Send + Sync + 'static>>,
}

impl Daemons {
    pub fn new() -> Self {
        Self { velkard: None, cpu_miner: None }
    }

    pub fn with_velkard(mut self, velkard: Arc<dyn VelkardCtl + Send + Sync + 'static>) -> Self {
        self.velkard = Some(velkard);
        self
    }
    ...
    pub fn velkard(&self) -> Arc<dyn VelkardCtl + Send + Sync + 'static> {
        self.velkard.as_ref().expect("accessing Daemons::velkard while velkard option is None").clone()
    }

    pub fn try_velkard(&self) -> Option<Arc<dyn VelkardCtl + Send + Sync + 'static>> {
        self.velkard.clone()
    }

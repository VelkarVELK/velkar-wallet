...
impl Default for VelkardConfig {
    fn default() -> Self {
        Self {
            mute: false,
            path: None,
...
impl TryFrom<VelkardConfig> for Vec<String> {
    type Error = Error;
    fn try_from(args: VelkardConfig) -> Result<Vec<String>> {
        let mut argv = Vec::new();

        if args.path.is_none() {
            return Err(Error::Custom("no velkard path is specified".to_string()));
        }
...
impl Velkard {
    pub fn new(args: VelkardConfig) -> Self {
        Self {
    ...
    pub fn configure(&self, config: VelkardConfig) -> Result<()> {
    ...
    pub fn start(&self) -> Result<()> {
        ...
        if let Some(process) = process
            && process.is_running()
        {
            return Err(Error::Custom("Velkar node is already running.".to_string()));
        }
        ...
        // let options = VelkardOptions::new(path,network)?;
    ...
    pub async fn version(&self) -> Result<String> {
        ...
        if let Some(path) = path {
            ...
            Ok("Velkard binary is not configured. Please use 'node select' command.".to_string())
        }
    }
}

#[async_trait]
pub trait VelkardCtl {
    async fn version(&self) -> Result<String>;
    async fn configure(&self, config: VelkardConfig) -> Result<()>;

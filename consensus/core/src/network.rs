...
pub enum NetworkIdError {
    #[error("Invalid network name prefix: {0}. The expected prefix is 'velkar'.")]
...
///
/// NetworkId is a unique identifier for a velkar network instance.
...
impl NetworkId {
    ...
    /// Returns a textual description of the network prefixed with `velkar-`
    pub fn to_prefixed(&self) -> String {
        format!("velkar-{}", self)
    }

    pub fn from_prefixed(prefixed: &str) -> Result<Self, NetworkIdError> {
        if let Some(stripped) = prefixed.strip_prefix("velkar-") {

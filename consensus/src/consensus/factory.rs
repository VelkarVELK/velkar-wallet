...
use velkar_consensus_core::{api::ConsensusApi, config::Config, mining_rules::MiningRules};
use velkar_consensus_notify::root::ConsensusNotificationRoot;
use velkar_consensusmanager::{ConsensusFactory, ConsensusInstance, DynConsensusCtl, SessionLock};
use velkar_core::{debug, time::unix_now, warn};
use velkar_database::{
    prelude::{
...
impl ConsensusFactory for Factory {
    fn new_active_consensus(&self) -> (ConsensusInstance, DynConsensusCtl) {
        ...
        let db = velkar_database::prelude::ConnBuilder::default()
            .with_db_path(dir)

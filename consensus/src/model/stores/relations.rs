...
use velkar_consensus_core::BlockHashSet;
use velkar_consensus_core::{BlockHashMap, BlockHasher, BlockLevel, blockhash::BlockHashes};
use velkar_database::prelude::{BatchDbWriter, CachePolicy, DbWriter};
use velkar_database::prelude::{CachedDbAccess, DbKey, DirectDbWriter};
use velkar_database::prelude::{DB, StoreResult};
use velkar_database::prelude::{DirectWriter, MemoryWriter};
use velkar_database::prelude::{ReadLock, StoreError};
use velkar_database::registry::{DatabaseStorePrefixes, SEPARATOR};
...
impl RelationsStoreReader for StagingRelationsStore<'_> {
    ...
    fn counts(&self) -> Result<(usize, usize), StoreError> {
        let count = self
            ...
            .iterator()
            .map(|r| r.unwrap().0)
            .map(|k| <[u8; velkar_hashes::HASH_SIZE]>::try_from(&k[..]).unwrap())
            .map(Hash::from_bytes)
...
mod tests {
    ...
    use velkar_utils::mem_size::MemMode;
    ...
    fn test_db_relations_store() {
        let (lt, db) = create_temp_db!(velkar_database::prelude::ConnBuilder::default().with_files_limit(10));

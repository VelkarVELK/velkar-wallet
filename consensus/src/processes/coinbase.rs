...
const VELKAR_INITIAL_SUBSIDY: u64 = 2 * velkar_consensus_core::constants::SOMPI_PER_VELKAR;
...
impl CoinbaseManager {
    pub fn new(
        ...
        // In a 10 BPS network, the induced increase in total rewards is 51 VELK (see tests::calc_high_bps_total_rewards_delta())
        let bps_before = bps_history.before().max(1);
        let bps_after = bps_history.after().max(1);
        let subsidy_by_month_table_before: SubsidyByMonthTable =
            core::array::from_fn(|i| SUBSIDY_BY_MONTH_TABLE[i].div_ceil(bps_before));
        let subsidy_by_month_table_after: SubsidyByMonthTable =
            core::array::from_fn(|i| SUBSIDY_BY_MONTH_TABLE[i].div_ceil(bps_after));
        Self {
    ...
    pub fn expected_coinbase_transaction<T: AsRef<[u8]>>(
        ...
        // Add an output for each mergeset blue block (âˆ© DAA window), paying to the script reported by the block.
        ...
        // Collect all rewards from mergeset reds âˆ© DAA window and create a
    ...
    pub fn calc_block_subsidy(&self, daa_score: u64) -> u64 {
        // Velkar schedule: fixed 2 VELK initial subsidy with annual halving.
        // DAA score is used as block height proxy.
        if daa_score < self.deflationary_phase_daa_score {
            return self.pre_deflationary_phase_base_subsidy;
        }
        let blocks_per_halving = (SECONDS_PER_YEAR * 1000) / self.target_time_per_block_ms.max(1);
        let halvings = ((daa_score - self.deflationary_phase_daa_score) / blocks_per_halving) as u32;
        if halvings >= 64 {
            0
        } else {
            VELKAR_INITIAL_SUBSIDY >> halvings
        }
    }
    ...
    fn subsidy_month(&self, daa_score: u64) -> u64 {
        let seconds_since_deflationary_phase_started = if self.crescendo_activation_daa_score < self.deflationary_phase_daa_score {
            ...
            (daa_score - self.deflationary_phase_daa_score) / self.bps_history.after().max(1)
        } else if daa_score < self.crescendo_activation_daa_score {
            // deflationary_phase <= daa_score < crescendo_activation (pre activation)
            (daa_score - self.deflationary_phase_daa_score) / self.bps_history.before().max(1)
        } else {
            // Else - deflationary_phase <= crescendo_activation <= daa_score.
            // Count seconds differently before and after Crescendo activation
            (self.crescendo_activation_daa_score - self.deflationary_phase_daa_score) / self.bps_history.before().max(1)
                + (daa_score - self.crescendo_activation_daa_score) / self.bps_history.after().max(1)
        };
...
/*
    This table was pre-calculated by calling `calcDeflationaryPeriodBlockSubsidyFloatCalc` (in velkard-go) for all months until reaching 0 subsidy.
...
mod tests {
    ...
    fn calc_high_bps_total_rewards_delta() {
        ...
        println!("Total rewards: {} sompi => {} VELK", total_rewards, total_rewards / SOMPI_PER_VELKAR);
        println!("Total high bps rewards: {} sompi => {} VELK", total_high_bps_rewards, total_high_bps_rewards / SOMPI_PER_VELKAR);
        println!("Delta: {} sompi => {} VELK", delta, delta / SOMPI_PER_VELKAR as i64);
    }
    ...
    /// `cargo test --release --package velkar-consensus --lib -- processes::coinbase::tests::verify_crescendo_emission_schedule --exact --nocapture --ignored`
    #[test]
    #[ignore = "long"]
    fn verify_crescendo_emission_schedule() {
        ...
        for network_id in [NetworkId::new(NetworkType::Mainnet)] {
            ...
            for activation in activations {
                ...
                let diff = (new_total as i64 - baseline_total as i64) / SOMPI_PER_VELKAR as i64;
                assert!(diff.abs() <= 51, "activation: {}", activation);
                println!("DIFF (VELK): {}", diff);
            }
        }
    }
    ...
    #[test]
    fn subsidy_test() {
        const SECONDS_PER_HALVING: u64 = SECONDS_PER_YEAR;

        fn expected_subsidy(initial: u64, halvings: u32) -> u64 {
            if halvings >= 64 { 0 } else { initial >> halvings }
        }
        ...
        for network_id in NetworkId::iter() {
            ...
            let cbm = create_manager(&params);
            let initial_subsidy = params.pre_deflationary_phase_base_subsidy;
            let blocks_per_halving = (SECONDS_PER_HALVING * 1000) / params.target_time_per_block();
            ...
            let mut tests = vec![
                ...
                Test {
                    ...
                    expected: expected_subsidy(initial_subsidy, 1),
                },
                Test {
                    name: "after 2 halvings",
                    daa_score: params.deflationary_phase_daa_score + 2 * blocks_per_halving,
                    expected: expected_subsidy(initial_subsidy, 2),
                },
                Test {
                    name: "after 5 halvings",
                    daa_score: params.deflationary_phase_daa_score + 5 * blocks_per_halving,
                    expected: expected_subsidy(initial_subsidy, 5),
                },
                Test {
                    name: "after 27 halvings",
                    daa_score: params.deflationary_phase_daa_score + 27 * blocks_per_halving,
                    expected: expected_subsidy(initial_subsidy, 27),
                },
                Test {
                    name: "after 28 halvings",
                    daa_score: params.deflationary_phase_daa_score + 28 * blocks_per_halving,
                    expected: expected_subsidy(initial_subsidy, 28),
                },
                Test {
    ...
    fn create_legacy_manager() -> CoinbaseManager {
        CoinbaseManager::new(150, 204, 15778800 - 259200, 50000000000, ForkedParam::new_const(1), 1000)
    }
}

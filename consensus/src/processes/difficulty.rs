...
use velkar_core::{info, log::CRESCENDO_KEYWORD};
use velkar_hashes::Hash;
use velkar_math::{Uint256, Uint320};
use std::{
    cmp::{Ordering, max},
    ops::Deref,
    sync::{
...
/// A difficulty manager based on sampled block windows, implementing [KIP-0004](https://github.com/velkarnet/kips/blob/master/kip-0004.md)
#[derive(Clone)]
...
impl<T: HeaderStoreReader, U: GhostdagStoreReader> SampledDifficultyManager<T, U> {
    ...
    pub fn calculate_difficulty_bits(&self, window: &BlockWindowHeap, ghostdag_data: &GhostdagData) -> u32 {
        ...
        let raw_target = average_target * measured_duration / expected_duration;

        // Velkar retargets every block. Bound each move so a short run of lucky
        // blocks cannot make the next block practically impossible, while still
        // allowing the network to converge as hashrate changes.
        let min_target = average_target / 2u64;
        let max_target = average_target * 2u64;
        let new_target = raw_target.max(min_target).min(max_target);
        let genesis_target = Uint320::from(Uint256::from_compact_target_bits(self.genesis_bits));

        // Do not allow DAA to relax below the genesis difficulty. This keeps a
        // freshly bootstrapped Velkar network from becoming easier than the
        // calibrated launch target after the first small sample window.
        Uint256::try_from(new_target.min(self.max_difficulty_target).min(genesis_target))
...
#[cfg(test)]
mod tests {
    use velkar_consensus_core::{BlockLevel, BlueWorkType, MAX_WORK_LEVEL};
    use velkar_math::{Uint256, Uint320};
    use velkar_pow::calc_level_from_pow;

    use crate::processes::difficulty::{calc_work, level_work};
    use velkar_utils::hex::ToHex;

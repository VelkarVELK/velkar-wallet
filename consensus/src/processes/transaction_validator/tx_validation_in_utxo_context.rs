...
use velkar_txscript::{SigCacheKey, TxScriptEngine, caches::Cache};
    ...
    fn check_sequence_lock(tx: &impl VerifiableTransaction, pov_daa_score: u64) -> TxResult<()> {
        ...
        if tx.populated_inputs().filter(|(input, _)| input.sequence & SEQUENCE_LOCK_TIME_DISABLED != SEQUENCE_LOCK_TIME_DISABLED).any(
            |(input, entry)| {
                ...
                //
                // Note: in the velkard codebase there's a use in i64 in order to use the -1 value
...
mod tests {
    ...
    use velkar_consensus_core::tx::{MutableTransaction, PopulatedTransaction, ScriptVec, TransactionId, UtxoEntry};
    use velkar_consensus_core::tx::{ScriptPublicKey, Transaction, TransactionInput, TransactionOutpoint, TransactionOutput};
    use velkar_txscript_errors::TxScriptError;

...
pub fn get_velkar_tempdir() -> TempDir {
    let global_tempdir = std::env::temp_dir();
    let velkar_tempdir = global_tempdir.join("velkar");
    std::fs::create_dir_all(velkar_tempdir.as_path()).unwrap();
    tempfile::tempdir_in(velkar_tempdir.as_path()).unwrap()
}

/// Creates a DB within a temp directory under `<OS SPECIFIC TEMP DIR>/velkar`
/// Callers must keep the `TempDbLifetime` guard for as long as they wish the DB to exist.
#[macro_export]
macro_rules! create_temp_db {
    ($conn_builder: expr) => {{
        let db_tempdir = $crate::utils::get_velkar_tempdir();

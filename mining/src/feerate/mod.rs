...
impl FeerateEstimatorArgs {
    pub fn new(network_blocks_per_second: f64, maximum_mass_per_block: u64) -> Self {
        Self { network_blocks_per_second, maximum_mass_per_block }
    }

    pub fn network_mass_per_second(&self) -> u64 {
        (self.network_blocks_per_second * self.maximum_mass_per_block as f64).round().max(1.0) as u64
    }
}
...
impl FeerateEstimator {
    pub fn new(total_weight: f64, inclusion_interval: f64, target_time_per_block_seconds: f64) -> Self {
        ...
        let inclusion_interval = inclusion_interval.clamp(0.0, 1.0 - f64::EPSILON);

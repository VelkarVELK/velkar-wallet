...
use velkar_consensusmanager::{ConsensusProxy, spawn_blocking};
use velkar_core::{debug, error, info, time::Stopwatch, warn};
use velkar_mining_errors::{manager::MiningManagerError, mempool::RuleError};
...
impl MiningManager {
                ...
                Err(BuilderError::ConsensusError(BlockRuleError::InvalidTransactionsInNewBlock(invalid_transactions))) => {
                    ...
                    invalid_transactions.iter().for_each(|(x, err)| {
                        ...
                        if let Err(err) = removal_result {
                            ...
                            // https://github.com/velkarnet/velkard/issues/1553
    ...
    pub(crate) fn get_realtime_feerate_estimations_verbose(
        ...
        prefix: velkar_addresses::Prefix,
        ...
        {
            let script_public_key = velkar_txscript::pay_to_address_script(&velkar_addresses::Address::new(
                prefix,
                velkar_addresses::Version::PubKey,
                &[0u8; 32],
            ));
            let miner_data: MinerData = MinerData::new(script_public_key, vec![]);

            let BlockTemplate { block: velkar_consensus_core::block::MutableBlock { transactions, .. }, calculated_fees, .. } =
...
mod tests {
    use super::*;
    use velkar_consensus_core::subnets;

...
use velkar_consensus_core::{hashing::sighash::SigHashReusedValuesUnsync, mass::NonContextualMasses};
use velkar_txscript::{get_sig_op_count_upper_bound, is_unspendable, script_class::ScriptClass};
...
impl Mempool {
    pub(crate) fn check_transaction_standard_in_isolation(&self, transaction: &MutableTransaction) -> NonStandardResult<()> {
        ...
        // However, in a later version of Velkar the consensus-valid transaction version range might diverge from the
...
mod tests {
    ...
    fn test_check_transaction_standard_in_isolation() {
        ...
        let dummy_script_public_key = velkar_txscript::pay_to_address_script(&addr);

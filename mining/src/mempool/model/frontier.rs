...
use velkar_consensus_core::{block::TemplateTransactionSelector, tx::Transaction};
use velkar_core::trace;
...
impl Frontier {
    ...
    pub fn insert(&mut self, key: FeerateTransactionKey) -> bool {
        ...
        if self.search_tree.insert(key) {
            ...
            // A decaying average formula. Denote É› = 1 - AVG_MASS_DECAY_FACTOR. A transaction inserted N slots ago has
            // É› * (1 - É›)^N weight within the updated average. This gives some weight to the full mempool history while
    ...
    ///           from `~1/50` (2000 sompi for a transaction with 100K storage mass), to `5M` (100 VELK fee for a transaction with
    ///           2000 mass = 100Â·100_000_000/2000), resulting in a range of size 250M (`5M/(1/50)`).
    ...
    /// full transaction selection in less than 150 Âµs even if the frontier has 1M entries (!!). See mining/benches
    /// for more details.  
    ...
    pub fn build_feerate_estimator(&self, args: FeerateEstimatorArgs) -> FeerateEstimator {
        ...
        let bps = args.network_blocks_per_second.max(f64::EPSILON);
        ...
        while let Some(current) = down_iter.next() {
            ...
            // Slow block-rate networks can legitimately produce intervals close to, or above, one second,
            // so the estimator clamps the value into a safe range instead of panicking.
...
mod tests {
    ...
    pub fn test_highly_irregular_sampling() {
        ...
        for i in 0..cap as u64 {
            ...
            if i == 0 {
                ...
                fee = 100_000_000 * 1_000_000; // 1M VELK
            }
            let mass: u64 = 1650;
    ...
    fn test_feerate_estimator() {
        ...
        for len in [0, 1, 10, 100, 200, 300, 500, 750, cap / 2, (cap * 2) / 3, (cap * 4) / 5, (cap * 5) / 6, cap] {
            ...
            let args = FeerateEstimatorArgs { network_blocks_per_second: 1.0, maximum_mass_per_block: 500_000 };

...
pub fn register_link_matchers(cli: &Arc<VelkarCli>) -> Result<()> {
    if !is_wasm() {
        return Ok(());
    }
    ...
    let cli_ = cli.clone();
    cli.term().register_link_matcher(
        &js_sys::RegExp::new(r"(velkar|velkartest|velkardev|velkarsim):\S+", "i"),
        Arc::new(Box::new(move |modifiers, uri| {
            if modifiers.ctrl || modifiers.meta {
                write_to_clipboard(&cli_, uri);
                cli_.term().writeln("Velkar explorer is not configured yet. Address copied to clipboard.");
            } else {
                write_to_clipboard(&cli_, uri);
            }
        })),
    ...
    cli.term().register_link_matcher(
        ...
        Arc::new(Box::new(move |modifiers, text| {
            ...
            if modifiers.ctrl || modifiers.meta {
                ...
                cli_.term().writeln("Velkar explorer is not configured yet. Block hash copied to clipboard.");
            } else {
                write_to_clipboard(&cli_, uri.to_string().as_str());
            }
        })),
    ...
    cli.term().register_link_matcher(
        ...
        Arc::new(Box::new(move |modifiers, text| {
            ...
            if modifiers.ctrl || modifiers.meta {
                write_to_clipboard(&cli_, uri.to_string().as_str());
                cli_.term().writeln("Velkar explorer is not configured yet. Tx id copied to clipboard.");
            } else {
                write_to_clipboard(&cli_, uri.to_string().as_str());
            }
        })),
...
fn write_to_clipboard(cli: &Arc<VelkarCli>, text: &str) {
    if is_nw() {

...
use velkar_daemon::{DaemonEvent, DaemonKind, Daemons};
use velkar_wallet_core::account::Account;
use velkar_wallet_core::rpc::DynRpcApi;
use velkar_wallet_core::storage::{IdT, PrvKeyDataInfo};
use velkar_wrpc_client::{VelkarRpcClient, Resolver};
...
impl From<&VelkarCli> for Arc<Terminal> {
    fn from(ctx: &VelkarCli) -> Arc<Terminal> {
        ctx.term()
    }
}

impl AsRef<VelkarCli> for VelkarCli {
    fn as_ref(&self) -> &Self {
        self
    }
}

impl workflow_log::Sink for VelkarCli {
...
impl VelkarCli {
    pub fn init() {
        cfg_if! {
            if #[cfg(not(target_arch = "wasm32"))] {
                ...
                velkar_core::log::set_log_level(LevelFilter::Info);
            }
        }
    ...
    pub async fn try_new_arc(options: Options) -> Result<Arc<Self>> {
        ...
        let velkar_cli = Arc::new(VelkarCli {
        ...
        let term = Arc::new(Terminal::try_new_with_options(velkar_cli.clone(), options.terminal)?);
        term.init().await?;

        cfg_if! {
            if #[cfg(target_arch = "wasm32")] {
                velkar_cli.init_panic_hook();
            }
        }
    ...
    pub fn try_rpc_client(&self) -> Option<Arc<VelkarRpcClient>> {
                    ...
                    msg = multiplexer.receiver.recv().fuse() => {
                        ...
                        if let Ok(msg) = msg {
                            match *msg {
                                Events::WalletList { .. } => {},
                                Events::WalletPing => {
                                    // log_info!("Velkar NG - received wallet ping");
                                },
                                Events::Metrics { network_id : _, metrics : _ } => {
                                    // log_info!("Velkar NG - received metrics event {metrics:?}")
                                }
                                Events::FeeRate { .. } => {},
                                ...
                                Events::UtxoIndexNotEnabled { .. } => {
                                    tprintln!(this, "Error: Velkar node UTXO index is not enabled...")
                                },
                                ...
                                Events::ServerStatus {
                                    ...
                                    tprintln!(this, "Connected to Velkar node version {server_version} at {}", url.unwrap_or("N/A".to_string()));

                                    let is_open = this.wallet.is_open();

                                    if !is_synced {
                                        if is_open {
                                            terrorln!(this, "Unable to update the wallet state - Velkar node is currently syncing with the network...");

                                        } else {
                                            terrorln!(this, "Velkar node is currently syncing with the network, please wait for the sync to complete...");
                                        }
                                    }
    ...
    pub async fn shutdown(&self) -> Result<()> {
        if !self.shutdown.load(Ordering::SeqCst) {
            ...
            let velkard = self.daemons().try_velkard();
            ...
            if let Some(velkard) = velkard.as_ref() {
                velkard.mute(false).await?;
                velkard.stop().await?;
            }
...
impl Cli for VelkarCli {
    ...
    fn prompt(&self) -> Option<String> {
        ...
        if let Some(descriptor) = self.wallet.descriptor() {
            ...
            if title.to_lowercase().as_str() != "velkar" {
                prompt.push(title);
            }
...
impl cli::Context for VelkarCli {
...
pub async fn velkar_cli(terminal_options: TerminalOptions, banner: Option<String>) -> Result<()> {
    VelkarCli::init();

    let options = Options::new(terminal_options, None);
    let cli = VelkarCli::try_new_arc(options).await?;

    let banner =
        banner.unwrap_or_else(|| format!("Velkar CLI Wallet v{} (type 'help' for list of commands)", env!("CARGO_PKG_VERSION")));

...
const VELKAR_LOCAL_WRPC_PORT: u16 = 17110;

fn normalize_velkar_local_url(url: &str) -> String {
    let trimmed = url.trim();
    let without_scheme = trimmed.strip_prefix("ws://").unwrap_or(trimmed);
    let without_scheme = without_scheme.strip_prefix("wss://").unwrap_or(without_scheme);

    if without_scheme.eq_ignore_ascii_case("localhost") || without_scheme == "127.0.0.1" {
        format!("127.0.0.1:{VELKAR_LOCAL_WRPC_PORT}")
    } else {
        trimmed.to_string()
    }
}
...
#[help("Connect to a Velkar network")]
pub struct Connect;
    ...
    async fn main(self: Arc<Self>, ctx: &Arc<dyn Context>, argv: Vec<String>, _cmd: &str) -> Result<()> {
        ...
        if let Some(wrpc_client) = ctx.wallet().try_wrpc_client().as_ref() {
            ...
            let (is_public, url) = match arg_or_server_address.as_deref() {
                ...
                Some(url) => {
                    let url = normalize_velkar_local_url(url);
                    (false, wrpc_client.parse_url_with_network_type(url, network_id.into()).map_err(|e| e.to_string())?)
                }
            };

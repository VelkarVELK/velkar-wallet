use crate::imports::*;
use velkar_daemon::VelkardConfig;
...
pub struct Node {
    settings: SettingsStore<VelkardSettings>,
    mute: Arc<AtomicBool>,
...
impl Default for Node {
    fn default() -> Self {
        Node {
            settings: SettingsStore::try_new("velkard").expect("Failed to create node settings store"),
...
impl Handler for Node {
    fn verb(&self, ctx: &Arc<dyn Context>) -> Option<&'static str> {
        if let Ok(ctx) = ctx.clone().downcast_arc::<VelkarCli>() { ctx.daemons().clone().velkard.as_ref().map(|_| "node") } else { None }
    }

    fn help(&self, _ctx: &Arc<dyn Context>) -> &'static str {
        "Manage the local Velkar node instance"
    }

    async fn start(self: Arc<Self>, _ctx: &Arc<dyn Context>) -> cli::Result<()> {
        self.settings.try_load().await.ok();
        if let Some(mute) = self.settings.get(VelkardSettings::Mute) {
    ...
    async fn handle(self: Arc<Self>, ctx: &Arc<dyn Context>, argv: Vec<String>, cmd: &str) -> cli::Result<()> {
        let ctx = ctx.clone().downcast_arc::<VelkarCli>()?;
...
impl Node {
    ...
    async fn create_config(&self, ctx: &Arc<VelkarCli>) -> Result<VelkardConfig> {
        let location: String = self
            .settings
            .get(VelkardSettings::Location)
        ...
        let mute = false;
        let config = VelkardConfig::new(location.as_str(), network_id, mute);
        Ok(config)
    }

    async fn main(self: Arc<Self>, ctx: Arc<VelkarCli>, mut argv: Vec<String>, cmd: &str) -> Result<()> {
        if argv.is_empty() {
            return self.display_help(ctx, argv).await;
        }
        let velkard = ctx.daemons().velkard();
        match argv.remove(0).as_str() {
            "start" => {
                let mute = self.mute.load(Ordering::SeqCst);
                if mute {
                    tprintln!(ctx, "starting velkar node... {}", style("(logs are muted, use 'node mute' to toggle)").dim());
                } else {
                    tprintln!(ctx, "starting velkar node... {}", style("(use 'node mute' to mute logging)").dim());
                }

                let wrpc_client = ctx.wallet().try_wrpc_client().ok_or(Error::custom("Unable to start node with non-wRPC client"))?;

                velkard.configure(self.create_config(&ctx).await?).await?;
                velkard.start().await?;
            ...
            "mute" | "logs" => {
                ...
                // velkard.mute(mute).await?;
                self.settings.set(VelkardSettings::Mute, mute).await?;
            }
            "status" => {
                let status = velkard.status().await?;
                tprintln!(ctx, "{}", status);
            }
            "select" => {
            ...
            "version" => {
                velkard.configure(self.create_config(&ctx).await?).await?;
                let version = velkard.version().await?;
    ...
    async fn display_help(self: Arc<Self>, ctx: Arc<VelkarCli>, _argv: Vec<String>) -> Result<()> {
        ctx.term().help(
            &[
                ("select", "Select Velkard executable (binary) location"),
                ("version", "Display Velkard executable version"),
                ("start", "Start the local Velkar node instance"),
                ("stop", "Stop the local Velkar node instance"),
                ("restart", "Restart the local Velkar node instance"),
                ("kill", "Kill the local Velkar node instance"),
                ("status", "Get the status of the local Velkar node instance"),
    ...
    async fn select(self: Arc<Self>, ctx: Arc<VelkarCli>, path: Option<String>) -> Result<()> {
        let root = nw_sys::app::folder();

        match path {
            None => {
                let binaries = velkar_daemon::locate_binaries(root.as_str(), "velkard").await?;

                if binaries.is_empty() {
                    tprintln!(ctx, "No velkard binaries found");
                } else {
                    let binaries = binaries.iter().map(|p| p.display().to_string()).collect::<Vec<_>>();
                    if let Some(selection) = ctx.term().select("Please select a velkard binary", &binaries).await? {
                        tprintln!(ctx, "selecting: {}", selection);
                        self.settings.set(VelkardSettings::Location, selection.as_str()).await?;
                    } else {
            ...
            Some(path) => {
                if fs::exists(&path).await? {
                    ...
                    self.settings.set(VelkardSettings::Location, path.as_str()).await?;
                } else {
                    twarnln!(ctx, "destination binary not found, please specify full path including the binary name");
                    twarnln!(ctx, "example: 'node select /home/user/testnet/velkard'");
                    tprintln!(ctx, "no selection is made");
                }
            }
        }
    ...
    pub async fn handle_event(&self, ctx: &Arc<VelkarCli>, event: Event) -> Result<()> {
        let term = ctx.term();
        ...
        match event {
            ...
            Event::Exit(_code) => {
                tprintln!(ctx, "Velkard has exited");
                self.is_running.store(false, Ordering::SeqCst);
                term.refresh_prompt();
            }
            Event::Error(error) => {
                tprintln!(ctx, "{}", style(format!("Velkard error: {error}")).red());
                self.is_running.store(false, Ordering::SeqCst);
                term.refresh_prompt();
            }

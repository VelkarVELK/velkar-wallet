use crate::imports::*;
use velkar_daemon::{CpuMinerConfig, locate_binaries};
...
impl DefaultSettings for MinerSettings {
    async fn defaults() -> Vec<(Self, Value)> {
        ...
        if let Ok(binaries) = locate_binaries(&root, "velkar-cpu-miner").await
...
impl Handler for Miner {
    fn verb(&self, ctx: &Arc<dyn Context>) -> Option<&'static str> {
        if let Ok(ctx) = ctx.clone().downcast_arc::<VelkarCli>() {
...
impl Miner {
    ...
    async fn create_config(&self, ctx: &Arc<VelkarCli>) -> Result<CpuMinerConfig> {
    ...
    async fn main(self: Arc<Self>, ctx: Arc<VelkarCli>, mut argv: Vec<String>, _cmd: &str) -> Result<()> {
        if argv.is_empty() {
    ...
    async fn select(self: Arc<Self>, ctx: Arc<VelkarCli>) -> Result<()> {
        let root = nw_sys::app::folder();

        let binaries = velkar_daemon::locate_binaries(root.as_str(), "velkar-cpu-miner").await?;

        if binaries.is_empty() {
            tprintln!(ctx, "No velkar-cpu-miner binaries found");
        } else {
            let binaries = binaries.iter().map(|p| p.display().to_string()).collect::<Vec<_>>();
            if let Some(selection) = ctx.term().select("Please select velkar-cpu-miner binary", &binaries).await? {

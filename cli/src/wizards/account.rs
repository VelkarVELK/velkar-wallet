use crate::cli::VelkarCli;
use crate::imports::*;
use crate::result::Result;
use velkar_bip32::{Language, Mnemonic, WordCount};
use velkar_wallet_core::account::MULTISIG_ACCOUNT_KIND;
use velkar_wallet_core::storage::keydata::PrvKeyDataVariantKind;
// use velkar_wallet_core::runtime::wallet::AccountCreateArgsBip32;
// use velkar_wallet_core::runtime::{PrvKeyDataArgs, PrvKeyDataCreateArgs};
// use velkar_wallet_core::storage::AccountKind;
...
async fn create_multisig(ctx: &Arc<VelkarCli>, account_name: Option<String>, mnemonic_phrase_word_count: WordCount) -> Result<()> {
    let term = ctx.term();
    let wallet = ctx.wallet();
...
pub(crate) async fn bip32_watch(ctx: &Arc<VelkarCli>, name: Option<&str>) -> Result<()> {
    let term = ctx.term();
    let wallet = ctx.wallet();
...
pub(crate) async fn multisig_watch(ctx: &Arc<VelkarCli>, name: Option<&str>) -> Result<()> {
    let term = ctx.term();

use crate::{
    error::{Error, Result},
    resolver::{VelkardResponseReceiver, VelkardResponseSender, Resolver, matcher::Matcher},
};
...
struct Pending {
    ...
    sender: VelkardResponseSender,
}

impl Pending {
    fn new(op: VelkardPayloadOps, request: VelkardRequest, sender: VelkardResponseSender) -> Self {
        Self { timestamp: Instant::now(), op, request, sender }
    }

    fn is_matching(&self, response: &VelkardResponse, response_op: VelkardPayloadOps) -> bool {
...
impl Resolver for QueueResolver {
    fn register_request(&self, op: VelkardPayloadOps, request: &VelkardRequest) -> VelkardResponseReceiver {
        let (sender, receiver) = oneshot::channel::<Result<VelkardResponse>>();
        {
    ...
    fn handle_response(&self, response: VelkardResponse) {
        let response_op: VelkardPayloadOps = response.payload.as_ref().unwrap().try_into().expect("response is not a notification");

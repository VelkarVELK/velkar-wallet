...
use velkar_core::{debug, error, trace};
use velkar_grpc_core::{
    RPC_MAX_MESSAGE_SIZE,
    channel::NotificationChannel,
    ops::VelkardPayloadOps,
    protowire::{GetInfoRequestMessage, VelkardRequest, VelkardResponse, velkard_request, rpc_client::RpcClient},
};
use velkar_notify::{
...
use velkar_utils::{channel::Channel, triggers::DuplexTrigger};
use velkar_utils_tower::{
...
impl RpcApi for GrpcClient {
    ...
    //     self.inner.call(VelkardPayloadOps::SubmitBlock, request).await?.as_ref().try_into()
    // }
...
type VelkardRequestSender = async_channel::Sender<VelkardRequest>;
type VelkardRequestReceiver = async_channel::Receiver<VelkardRequest>;
...
/// Data flow:
/// ```
/// //   VelkardRequest -> request_send -> stream -> VelkardResponse
/// ```
///
/// Execution flow:
/// ```
...
struct Inner {
    ...
    // Sending to server
    request_sender: VelkardRequestSender,
    request_receiver: VelkardRequestReceiver,
...
impl Inner {
    ...
    async fn try_connect(
        ...
        request_timeout: u64,
        counters: Arc<TowerConnectionCounters>,
    ) -> Result<(Streaming<VelkardResponse>, ServerFeatures)> {
        // gRPC endpoint
        ...
        // Actual VelkardRequest to VelkardResponse stream
        let mut stream: Streaming<VelkardResponse> = client.message_stream(request_stream).await?.into_inner();
    ...
    async fn call(&self, op: VelkardPayloadOps, request: impl Into<VelkardRequest>) -> Result<VelkardResponse> {
        // Calls are only allowed if the client is connected to the server
        if self.is_connected() {
            let id = u64::from_le_bytes(rand::random::<[u8; 8]>());
            let mut request: VelkardRequest = request.into();
            request.id = id;
    ...
    fn spawn_response_receiver_task(self: Arc<Self>, mut stream: Streaming<VelkardResponse>) {
        // Note: self is a cloned Arc here so that it can be used in the spawned task.
    ...
    async fn start_notify_to_client(&self, scope: Scope) -> RpcResult<()> {
        let request = velkard_request::Payload::from_notification_type(&scope, Command::Start);
    ...
    async fn stop_notify_to_client(&self, scope: Scope) -> RpcResult<()> {
        if self.handle_stop_notify() {
            let request = velkard_request::Payload::from_notification_type(&scope, Command::Stop);
            self.call((&request).into(), request).await?;
        }
        Ok(())
    }

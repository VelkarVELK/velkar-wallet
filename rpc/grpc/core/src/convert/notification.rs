use crate::protowire::{
    BlockAddedNotificationMessage, VelkardResponse, NewBlockTemplateNotificationMessage, RpcNotifyCommand, velkard_response::Payload,
};
use crate::protowire::{
...
use velkar_rpc_core::{Notification, RpcError, RpcHash};
use std::str::FromStr;
use std::sync::Arc;
...
from!(item: &velkar_rpc_core::Notification, VelkardResponse, { Self { id: 0, payload: Some(item.into()) } });

from!(item: &velkar_rpc_core::Notification, Payload, {
    match item {
...
from!(item: &velkar_rpc_core::BlockAddedNotification, BlockAddedNotificationMessage, { Self { block: Some((&*item.block).into()) } });

from!(&velkar_rpc_core::NewBlockTemplateNotification, NewBlockTemplateNotificationMessage);

from!(item: &velkar_rpc_core::VirtualChainChangedNotification, VirtualChainChangedNotificationMessage, {
    Self {
...
from!(item: &velkar_rpc_core::FinalityConflictNotification, FinalityConflictNotificationMessage, {
    Self { violating_block_hash: item.violating_block_hash.to_string() }
});

from!(item: &velkar_rpc_core::FinalityConflictResolvedNotification, FinalityConflictResolvedNotificationMessage, {
    Self { finality_block_hash: item.finality_block_hash.to_string() }
});

from!(item: &velkar_rpc_core::UtxosChangedNotification, UtxosChangedNotificationMessage, {
    Self {
...
from!(item: &velkar_rpc_core::SinkBlueScoreChangedNotification, SinkBlueScoreChangedNotificationMessage, {
    Self { sink_blue_score: item.sink_blue_score }
});

from!(item: &velkar_rpc_core::VirtualDaaScoreChangedNotification, VirtualDaaScoreChangedNotificationMessage, {
    Self { virtual_daa_score: item.virtual_daa_score }
});

from!(&velkar_rpc_core::PruningPointUtxoSetOverrideNotification, PruningPointUtxoSetOverrideNotificationMessage);
...
try_from!(item: &VelkardResponse, velkar_rpc_core::Notification, {
    item.payload
        .as_ref()
        .ok_or_else(|| RpcError::MissingRpcFieldError("VelkardResponse".to_string(), "payload".to_string()))?
        .try_into()?
});

try_from!(item: &Payload, velkar_rpc_core::Notification, {
    match item {
...
try_from!(item: &BlockAddedNotificationMessage, velkar_rpc_core::BlockAddedNotification, {
    Self {
        block: Arc::new(
            item.block
                .as_ref()
...
try_from!(&NewBlockTemplateNotificationMessage, velkar_rpc_core::NewBlockTemplateNotification);

try_from!(item: &VirtualChainChangedNotificationMessage, velkar_rpc_core::VirtualChainChangedNotification, {
    Self {
...
try_from!(item: &FinalityConflictNotificationMessage, velkar_rpc_core::FinalityConflictNotification, {
    Self { violating_block_hash: RpcHash::from_str(&item.violating_block_hash)? }
});

try_from!(item: &FinalityConflictResolvedNotificationMessage, velkar_rpc_core::FinalityConflictResolvedNotification, {
    Self { finality_block_hash: RpcHash::from_str(&item.finality_block_hash)? }
});

try_from!(item: &UtxosChangedNotificationMessage, velkar_rpc_core::UtxosChangedNotification, {
    Self {
...
try_from!(item: &SinkBlueScoreChangedNotificationMessage, velkar_rpc_core::SinkBlueScoreChangedNotification, {
    Self { sink_blue_score: item.sink_blue_score }
});

try_from!(item: &VirtualDaaScoreChangedNotificationMessage, velkar_rpc_core::VirtualDaaScoreChangedNotification, {
    Self { virtual_daa_score: item.virtual_daa_score }
});

try_from!(&PruningPointUtxoSetOverrideNotificationMessage, velkar_rpc_core::PruningPointUtxoSetOverrideNotification);

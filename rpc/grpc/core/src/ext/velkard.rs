use velkar_notify::{scope::Scope, subscription::Command};

use crate::protowire::{
    VelkardRequest, VelkardResponse, NotifyBlockAddedRequestMessage, NotifyFinalityConflictRequestMessage,
    ...
    velkard_request, velkard_response,
};

impl VelkardRequest {
    pub fn from_notification_type(scope: &Scope, command: Command) -> Self {
        VelkardRequest { id: 0, payload: Some(velkard_request::Payload::from_notification_type(scope, command)) }
    }
...
impl velkard_request::Payload {
    pub fn from_notification_type(scope: &Scope, command: Command) -> Self {
        match scope {
            Scope::BlockAdded(_) => {
                velkard_request::Payload::NotifyBlockAddedRequest(NotifyBlockAddedRequestMessage { command: command.into() })
            }
            Scope::NewBlockTemplate(_) => {
                velkard_request::Payload::NotifyNewBlockTemplateRequest(NotifyNewBlockTemplateRequestMessage {
                    command: command.into(),
                })
            }

            Scope::VirtualChainChanged(scope) => {
                velkard_request::Payload::NotifyVirtualChainChangedRequest(NotifyVirtualChainChangedRequestMessage {
                    command: command.into(),
                    include_accepted_transaction_ids: scope.include_accepted_transaction_ids,
                })
            }
            Scope::FinalityConflict(_) => {
                velkard_request::Payload::NotifyFinalityConflictRequest(NotifyFinalityConflictRequestMessage {
                    command: command.into(),
                })
            }
            ...
            Scope::UtxosChanged(scope) => velkard_request::Payload::NotifyUtxosChangedRequest(NotifyUtxosChangedRequestMessage {
                addresses: scope.addresses.iter().map(|x| x.into()).collect::<Vec<String>>(),
                command: command.into(),
            }),
            Scope::SinkBlueScoreChanged(_) => {
                velkard_request::Payload::NotifySinkBlueScoreChangedRequest(NotifySinkBlueScoreChangedRequestMessage {
                    command: command.into(),
                })
            }
            Scope::VirtualDaaScoreChanged(_) => {
                velkard_request::Payload::NotifyVirtualDaaScoreChangedRequest(NotifyVirtualDaaScoreChangedRequestMessage {
                    command: command.into(),
                })
            }
            Scope::PruningPointUtxoSetOverride(_) => {
                velkard_request::Payload::NotifyPruningPointUtxoSetOverrideRequest(NotifyPruningPointUtxoSetOverrideRequestMessage {
                    command: command.into(),
                })
            }
        }
    ...
    pub fn is_subscription(&self) -> bool {
        use crate::protowire::velkard_request::Payload;
        matches!(
            self,
...
impl velkard_response::Payload {
    pub fn is_notification(&self) -> bool {
        use crate::protowire::velkard_response::Payload;
        match self {

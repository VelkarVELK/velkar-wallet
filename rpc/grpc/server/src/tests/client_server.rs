...
use velkar_notify::scope::{NewBlockTemplateScope, Scope};
use velkar_rpc_core::api::rpc::RpcApi;
use velkar_utils::networking::{ContextualNetAddress, NetAddress};
use std::sync::Arc;

#[tokio::test]
async fn test_client_server_sanity_check() {
    velkar_core::log::try_init_logger("info, velkar_grpc_core=trace, velkar_grpc_server=trace, velkar_grpc_client=trace");

...
pub enum GrpcServerError {
    ...
    RpcApiError(#[from] velkar_rpc_core::error::RpcError),
    ...
    ClosedHandler(VelkardPayloadOps),
...
impl From<GrpcServerError> for velkar_rpc_core::error::RpcError {
    fn from(err: GrpcServerError) -> Self {
        match err {
            GrpcServerError::RpcApiError(err) => err,
            GrpcServerError::NotificationError(err) => err.into(),
            _ => velkar_rpc_core::error::RpcError::General(err.to_string()),
        }
    }
}

impl From<GrpcServerError> for velkar_notify::error::Error {
    fn from(err: GrpcServerError) -> Self {
        match err {
            GrpcServerError::RpcApiError(err) => velkar_notify::error::Error::General(err.to_string()),
            GrpcServerError::NotificationError(err) => err,
            _ => velkar_notify::error::Error::General(err.to_string()),
        }
    }
}
...
impl<T> From<TrySendError<T>> for GrpcServerError {
    fn from(_: TrySendError<T>) -> Self {
        velkar_notify::error::Error::ChannelSendError.into()
    }
}

...
use velkar_grpc_core::{
    ...
    protowire::{
        VelkardRequest, VelkardResponse,
...
impl Rpc for ConnectionHandler {
    type MessageStreamStream = Pin<Box<dyn Stream<Item = Result<VelkardResponse, tonic::Status>> + Send + Sync + 'static>>;

    /// Handle the new arriving client connection
    async fn message_stream(
        &self,
        request: Request<tonic::Streaming<VelkardRequest>>,

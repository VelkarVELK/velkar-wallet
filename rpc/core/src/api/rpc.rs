//!
//! The main [`RpcApi`] trait that defines all RPC methods available in the Rusty Velkar p2p node.
//!
...
use velkar_notify::{listener::ListenerId, scope::Scope, subscription::Command};
use std::sync::Arc;
...
pub trait RpcApi: Sync + Send + AnySync {
    ...
    /// Requests the list of known velkard addresses in the current network (mainnet, testnet, etc.)

...
use velkar_utils::{hex::ToHex, serde_bytes_fixed_ref};
...
/// Represents the ID of a Velkar transaction
...
impl From<velkar_consensus_client::TransactionOutpoint> for RpcTransactionOutpoint {
    fn from(outpoint: velkar_consensus_client::TransactionOutpoint) -> Self {
        TransactionOutpoint::from(outpoint).into()
    }
}

impl From<RpcTransactionOutpoint> for velkar_consensus_client::TransactionOutpoint {
    fn from(outpoint: RpcTransactionOutpoint) -> Self {
        TransactionOutpoint::from(outpoint).into()
    }
}

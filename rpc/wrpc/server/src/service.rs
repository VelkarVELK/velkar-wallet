...
use velkar_rpc_core::api::ops::RpcApiOps;
...
/// ### VelkarRpcHandler
///
/// [`VelkarRpcHandler`] is a handler struct that implements the [`RpcHandler`] trait
...
///
pub struct VelkarRpcHandler {
    pub server: Server,
    pub options: Arc<Options>,
}

impl VelkarRpcHandler {
    pub fn new(
        tasks: usize,
        ...
        VelkarRpcHandler { server: Server::new(tasks, encoding, core_service, options.clone()), options }
    }
}

#[async_trait]
impl RpcHandler for VelkarRpcHandler {
    ...
    async fn handshake(
        ...
        //     sender,
        //     receiver,
        //     Box::pin(|msg| if msg != "velkar" { Err(WebSocketError::NegotiationFailure) } else { Ok(()) }),
        // )
        // .await
...
pub struct WrpcService {
    ...
    options: Arc<Options>,
    server: RpcServer,
    rpc_handler: Arc<VelkarRpcHandler>,
...
impl WrpcService {
    ...
    pub fn new(
        ...
        let rpc_handler = Arc::new(VelkarRpcHandler::new(tasks, *encoding, core_service, options.clone()));

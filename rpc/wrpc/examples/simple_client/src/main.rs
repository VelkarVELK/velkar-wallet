// Example of simple client to connect with Velkar node using wRPC connection and collect some node and network basic data

use velkar_rpc_core::{GetBlockDagInfoResponse, GetServerInfoResponse, api::rpc::RpcApi};
use velkar_wrpc_client::{
    VelkarRpcClient, Resolver, WrpcEncoding,
...
async fn main() -> ExitCode {
    match check_node_status().await {
        Ok(_) => {
            println!("Well done! You successfully completed your first client connection to Velkar node!");
            ExitCode::SUCCESS
        }
        Err(error) => {
...
async fn check_node_status() -> Result<()> {
    ...
    // Verify your Velkar node is runnning with --rpclisten-borsh=0.0.0.0:17110 parameter
    ...
    // Define the network your Velkar node is connected to
    ...
    let client = VelkarRpcClient::new(encoding, url, resolver, selected_network, subscription_context)?;
    ...
    // Connect to selected Velkar node
    client.connect(Some(options)).await?;

    // Retrieve and show Velkar node information
    ...
    // Retrieve and show Velkar network information
    ...
    // Disconnect client from Velkar node

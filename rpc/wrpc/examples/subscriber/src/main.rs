// Basic example of a Velkar wRPC client that connects to a node
...
// Velkar RPC primitives
use velkar_wrpc_client::prelude::*;
// reuse wRPC Result type for convenience
use velkar_wrpc_client::result::Result;
...
impl Listener {
    pub fn try_new(network_id: NetworkId, url: Option<String>) -> Result<Self> {
        ...
        // Create a basic Velkar RPC client instance using Borsh encoding.
        let client = Arc::new(VelkarRpcClient::new_with_args(WrpcEncoding::Borsh, url.as_deref(), resolver, Some(network_id), None)?);

//!
//! Module implementing [`Resolver`] client for obtaining public Velkar wRPC endpoints.
//!
...
///
/// # Resolver - a client for obtaining public Velkar wRPC endpoints.
///
/// This client operates against [Velkar Resolver](https://github.com/aspectron/velkar-resolver) service
/// that provides load-balancing and failover capabilities for Velkar wRPC endpoints. The default
/// configuration allows access to public Velkar nodes, while custom configurations can be supplied
/// if you are running your own custom Velkar node cluster.
///
#[derive(Debug, Clone)]
pub struct Resolver {
    inner: Arc<Inner>,
...
impl Resolver {
    ...
    fn make_url(&self, url: &str, encoding: Encoding, network_id: NetworkId) -> String {
        ...
        format!("{url}/v{CURRENT_VERSION}/velkar/{network_id}/{tls}/wrpc/{encoding}")
    }
    ...
    /// Obtain a Velkar p2p [`NodeDescriptor`] from the resolver based on the supplied [`Encoding`] and [`NetworkId`].
    ...
    /// Returns a Velkar wRPC URL from the resolver based on the supplied [`Encoding`] and [`NetworkId`].

...
use velkar_consensus_core::api::counters::ProcessingCounters;
use velkar_consensus_core::daa_score_timestamp::DaaScoreTimestamp;
use velkar_consensus_core::errors::block::RuleError;
use velkar_consensus_core::tx::{TransactionQueryResult, TransactionType};
use velkar_consensus_core::utxo::utxo_inquirer::UtxoInquirerError;
...
use velkar_index_core::indexed_utxos::BalanceByScriptPublicKey;
use velkar_index_core::{
...
use velkar_mining::feerate::FeeEstimateVerbose;
use velkar_mining::model::tx_query::TransactionQuery;
use velkar_mining::{manager::MiningManagerProxy, mempool::tx::Orphan};
use velkar_notify::listener::ListenerLifespan;
use velkar_notify::subscription::context::SubscriptionContext;
use velkar_notify::subscription::{MutationPolicies, UtxosChangedMutationPolicy};
use velkar_notify::{
...
use velkar_p2p_flows::flow_context::FlowContext;
use velkar_p2p_lib::common::ProtocolError;
use velkar_p2p_mining::rule_engine::MiningRuleEngine;
use velkar_perf_monitor::{Monitor as PerfMonitor, counters::CountersSnapshot};
use velkar_rpc_core::{
...
use velkar_txscript::{extract_script_pub_key_address, pay_to_address_script};
use velkar_utils::expiring_cache::ExpiringCache;
use velkar_utils::sysinfo::SystemInfo;
use velkar_utils::{channel::Channel, triggers::SingleTrigger};
use velkar_utils_tower::counters::TowerConnectionCounters;
use velkar_utxoindex::api::UtxoIndexProxy;
...
/// A service implementing the Rpc API at velkar_rpc_core level.
///
...
pub struct RpcCoreService {
    ...
    fee_estimate_verbose_cache: ExpiringCache<velkar_mining::errors::MiningManagerResult<GetFeeEstimateExperimentalResponse>>,
...
NOTE: This error usually indicates an RPC conversion error between the node and the miner. This is likely to reflect using a NON-SUPPORTED miner.",
    ...
    async fn get_block_template_call(
        ...
        if request.pay_address.prefix != self.config.prefix() {
            return Err(velkar_addresses::AddressError::InvalidPrefix(request.pay_address.prefix.to_string()))?;
        }
        ...
        let script_public_key = velkar_txscript::pay_to_address_script(&request.pay_address);
    ...
    async fn estimate_network_hashes_per_second_call(
        ...
        const LEGACY_VIRTUAL: velkar_hashes::Hash = velkar_hashes::Hash::from_bytes([0xff; velkar_hashes::HASH_SIZE]);

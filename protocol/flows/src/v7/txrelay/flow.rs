...
use velkar_consensusmanager::ConsensusProxy;
...
use velkar_p2p_lib::{
    ...
    pb::{RequestTransactionsMessage, TransactionNotFoundMessage, velkard_message::Payload},
};
use std::sync::Arc;
...
impl RelayTransactionsFlow {
    ...
    pub fn invs_channel_size() -> usize {
        ...
        // Note: in go-velkard we have 10,000 for this channel combined with tx channel.
        4096
    }
            ...
            match res {
                ...
                Err(MiningManagerError::MempoolError(RuleError::RejectNonStandard(..))) => {
                    ...
                    if self.spam_counter.is_multiple_of(100) {
                        velkar_core::warn!("Peer {} has shared {} spam/non-standard txs ({:?})", self.router, self.spam_counter, res);
                    }
                }
                Err(_) => {}
            }

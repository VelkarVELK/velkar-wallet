...
use crate::pb::{ReadyMessage, VerackMessage, VersionMessage, velkard_message::Payload};
use crate::{IncomingRoute, VelkardMessagePayloadType, Router};
use crate::{common::ProtocolError, dequeue_with_timeout, make_message};
use velkar_core::debug;

/// Implements the Velkar peer-to-peer handshake protocol
pub struct VelkardHandshake<'a> {
    router: &'a Router,
...
impl<'a> VelkardHandshake<'a> {
    /// Builds the handshake object and subscribes to handshake messages
    pub fn new(router: &'a Router) -> Self {
        Self {
            router,
            version_receiver: router.subscribe(vec![VelkardMessagePayloadType::Version]),
            verack_receiver: router.subscribe(vec![VelkardMessagePayloadType::Verack]),
            ready_receiver: router.subscribe(vec![VelkardMessagePayloadType::Ready]),
        }
    }

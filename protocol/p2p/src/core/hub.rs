use crate::{ConnectionInitializer, Peer, Router, common::ProtocolError, pb::VelkardMessage};
use velkar_core::{debug, info, warn};
...
impl Hub {
    ...
    pub async fn send(&self, peer_key: PeerKey, msg: VelkardMessage) -> Result<bool, ProtocolError> {
    ...
    pub async fn broadcast(&self, msg: VelkardMessage, filter_peer: Option<PeerKey>) {
        let peers = self
            .peers
            .read()
            .values()
    ...
    pub async fn broadcast_to_some_peers(&self, msg: VelkardMessage, num_peers: usize) {
        assert!(num_peers > 0);
    ...
    pub async fn broadcast_many(&self, msgs: Vec<VelkardMessage>, filter_peer: Option<PeerKey>) {
        if msgs.is_empty() {
            return;
        }
        let peers = self

...
use crate::share_handler::VelkarApiTrait;
use anyhow::{Context, Result};
use velkar_addresses::Address;
use velkar_consensus_core::block::Block;
use velkar_grpc_client::GrpcClient;
use velkar_notify::{listener::ListenerId, scope::NewBlockTemplateScope};
use velkar_rpc_core::notify::mode::NotificationMode;
use velkar_rpc_core::{
...
impl VelkarApi {
    /// Create a new Velkar API client
    pub async fn new(
        address: String,
        coinbase_tag_suffix: Option<String>,
        mut shutdown_rx: watch::Receiver<bool>,
    ) -> Result<Arc<Self>> {
        info!("Connecting to Velkar node at {}", address);
        ...
        debug!("{} {}", LogColors::api("[API]"), LogColors::label("Establishing RPC connection to Velkar node:"));
        ...
        let client = loop {
            ...
            match res {
                ...
                Err(e) => {
                    ...
                    warn!(
                        "failed to connect to velkar node at {} (attempt {}): {}, retrying in {:.2}s",
                        grpc_address,
                        attempt,
                        e,
                        backoff.as_secs_f64()
    ...
    async fn start_stats_thread(self: Arc<Self>) {
        ...
        use velkar_rpc_core::{EstimateNetworkHashesPerSecondRequest, GetBlockDagInfoRequest};
        ...
        loop {
            ...
            let dag_response = match self.client.get_block_dag_info_call(None, GetBlockDagInfoRequest {}).await {
                Ok(r) => r,
                Err(e) => {
                    warn!("failed to get network hashrate from velkar, prom stats will be out of date: {}", e);
                    continue;
                }
            };
    ...
    pub async fn submit_block(&self, block: Block) -> Result<SubmitBlockResponse> {
        // Use velkar_consensus_core::hashing::header::hash() for block hash calculation
        // In Velkar, the block hash is the header hash (transactions are represented by hash_merkle_root in header)
        use velkar_consensus_core::hashing::header;
        ...
        debug!(
            "{} {}",
            LogColors::api("[API]"),
            LogColors::api(&format!("===== ATTEMPTING BLOCK SUBMISSION TO VELKAR NODE ===== Hash: {}", block_hash))
        );
            ...
            Ok(response) => {
                ...
                if !response.report.is_success() {
                    ...
                    warn!(
                        "{} {}",
                        LogColors::api("[API]"),
                        LogColors::validation(&format!("===== BLOCK REJECTED BY VELKAR NODE ===== Hash: {}", block_hash))
                    );
                    warn!(
                        "{} {} {}",
                        LogColors::api("[API]"),
                ...
                info!(
                    "{} {}",
                    LogColors::api("[API]"),
                    LogColors::block(&format!("===== BLOCK ACCEPTED BY VELKAR NODE ===== Hash: {}", block_hash))
                );
            ...
            Err(e) => {
                ...
                if error_str.contains("ErrDuplicateBlock") || error_str.contains("duplicate") {
                    warn!(
                        "{} {}",
                        LogColors::api("[API]"),
                        LogColors::validation(&format!("===== BLOCK REJECTED BY VELKAR NODE: STALE ===== Hash: {}", block_hash))
                    );
                    warn!(
                        "{} {} {}",
                        LogColors::api("[API]"),
                    ...
                    error!(
                        "{} {}",
                        LogColors::api("[API]"),
                        LogColors::error(&format!("===== BLOCK REJECTED BY VELKAR NODE: INVALID ===== Hash: {}", block_hash))
                    );
    ...
    pub async fn wait_for_sync_with_shutdown(&self, mut shutdown_rx: watch::Receiver<bool>) -> Result<()> {
        debug!("checking velkard sync state");
        ...
        loop {
            ...
            match sync_res {
                Ok(is_synced) => {
                    if is_synced {
                        debug!("velkard synced, starting server");
                        break;
                    }
                }
                Err(e) => {
            ...
            warn!("Velkar is not synced, waiting for sync before starting bridge");
                ...
                Err(e) => {
                    ...
                    if error_str.contains("Odd number of digits") {
                        return Err(anyhow::anyhow!(
                            "Failed to convert RPC block to Block after {} attempts: {} - This usually indicates a malformed hash field in the block template from the Velkar node. The block may have a hash with an odd-length hex string.",
                            max_retries,
                            error_str
                        ));
                    } else {
    ...
    pub async fn get_balances_by_addresses(&self, addresses: &[String]) -> Result<Vec<(String, u64)>> {
        ...
        let utxos = self
            .client
            .get_utxos_by_addresses_call(None, velkar_rpc_core::GetUtxosByAddressesRequest::new(addresses))
            .await
        ...
        tokio::spawn(async move {
            ...
            loop {
                ...
                if let Err(e) = api_clone.wait_for_sync().await {
                    error!("error checking velkard sync state, attempting reconnect: {}", e);
...
impl VelkarApiTrait for VelkarApi {
    async fn get_block_template(
        ...
        VelkarApi::get_block_template(self, wallet_addr, "", "").await.map_err(|e| {
    ...
    async fn submit_block(
        &self,
        block: Block,
    ) -> Result<velkar_rpc_core::SubmitBlockResponse, Box<dyn std::error::Error + Send + Sync>> {
        VelkarApi::submit_block(self, block)
            .await
    ...
    async fn get_balances_by_addresses(
        ...
        VelkarApi::get_balances_by_addresses(self, addresses)
            .await
            .map_err(|e| Box::new(std::io::Error::other(e.to_string())) as Box<dyn std::error::Error + Send + Sync>)
    }

    async fn get_current_block_color(&self, block_hash: &str) -> Result<bool, Box<dyn std::error::Error + Send + Sync>> {
        VelkarApi::get_current_block_color(self, block_hash)
            .await
            .map_err(|e| Box::new(std::io::Error::other(e.to_string())) as Box<dyn std::error::Error + Send + Sync>)
    }
}

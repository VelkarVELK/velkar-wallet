...
use velkar_stratum_bridge::{BridgeConfig, InstanceConfig, net_utils::normalize_port};
...
pub struct Cli {
    ...
    #[arg(last = true, help = "Velkard arguments (use '--' separator if velkard args start with hyphens)")]
    pub velkard_args: Vec<String>,

    #[arg(long)]
    pub velkard_address: Option<String>,
...
pub fn apply_cli_overrides(config: &mut BridgeConfig, cli: &Cli) -> Result<(), anyhow::Error> {
    if let Some(addr) = cli.velkard_address.as_deref() {

...
pub fn init_metrics() {
    ...
    BLOCK_ACCEPTED_COUNTER.get_or_init(|| {
        register_counter_vec!(
            ...
            "Number of blocks accepted by the connected Velkar node (may later be red)",
            WORKER_LABELS
        )
        .unwrap()
    });
    ...
    #[cfg(feature = "rkstratum_cpu_miner")]
    {
        ...
        INTERNAL_CPU_BLOCKS_ACCEPTED_TOTAL.get_or_init(|| {
            register_counter!(
                ...
                "Total blocks accepted by the connected Velkar node from the internal CPU miner since process start"
            )
            .unwrap()
        });
...
async fn handle_http_request(
    ...
    if request.starts_with("GET /api/status") {
        let velkard_version = crate::velkarapi::NODE_STATUS.lock().server_version.clone().unwrap_or_else(|| "-".to_string());
        ...
        let status =
            WebStatusResponse { velkard_address: status_cfg.velkard_address, velkard_version, instances: status_cfg.instances, web_bind };
...
struct WebStatusResponse {
    velkard_address: String,
    velkard_version: String,
    instances: usize,
    web_bind: String,
}
...
pub fn set_web_status_config(velkard_address: String, instances: usize) {
    let lock =
        WEB_STATUS_CONFIG.get_or_init(|| parking_lot::RwLock::new(WebStatusConfig { velkard_address: "-".to_string(), instances: 1 }));
    *lock.write() = WebStatusConfig { velkard_address, instances: instances.max(1) };
}

fn get_web_status_config() -> WebStatusConfig {
    WEB_STATUS_CONFIG
        .get_or_init(|| parking_lot::RwLock::new(WebStatusConfig { velkard_address: "-".to_string(), instances: 1 }))
        .read()
        .clone()
}
...
pub fn record_balances(instance_id: &str, balances: &[(String, u64)]) {
    if let Some(gauge) = BALANCE_GAUGE.get() {
        for (address, balance) in balances {
            // Convert from sompi to VELK (divide by 100000000)
...
/// Does not refresh dashboard activity â€” jobs alone must not keep 0-share workers "online".
...
async fn get_config_json() -> String {
        ...
        && let Ok(config) = BridgeConfig::from_yaml(&content)
    {
        ...
        let json_value = serde_json::json!({
            // Global fields
            "velkard_address": config.global.velkard_address,
...
async fn update_config_from_json(json_body: &str) -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
    ...
    if let Some(addr) = updates.get("velkard_address").and_then(|v| v.as_str()) {
        config.global.velkard_address = addr.to_string();
    }
...
"#,
        ...
        assert!(status_resp.contains("\"velkard_address\""));
        ...
        assert!(config_resp.contains("\"velkard_address\""));
        ...
        let json_body = r#"{"velkard_address":"127.0.0.2:16110","stratum_port":":5556","min_share_diff":4096}"#;

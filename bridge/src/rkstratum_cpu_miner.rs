...
struct Work {
    id: u64,
    block: Block,
    pow_state: Arc<velkar_pow::State>,
}
...
pub fn spawn_internal_cpu_miner(
    ...
    let velkar_api_submit = Arc::clone(&velkar_api);
    ...
    tokio::spawn(async move {
        while let Some(block) = submit_rx.recv().await {
            ...
            let res = velkar_api_submit.submit_block(block).await;
            match res {
                Ok(response) => {
                    if response.report.is_success() {
                        ...
                        let velkar_api_confirm = Arc::clone(&velkar_api_submit);
                        let metrics_confirm = Arc::clone(&metrics_submit);
                        tokio::spawn(async move {
                            for _ in 0..INTERNAL_BLOCK_CONFIRM_MAX_ATTEMPTS {
                                match velkar_api_confirm.get_current_block_color(&hash_str).await {
                                    Ok(true) => {
    ...
    let velkar_api_templates = Arc::clone(&velkar_api);
    ...
    tokio::spawn(async move {
        ...
        loop {
            ...
            match velkar_api_templates.get_block_template(&mining_address, "internal", "").await {
                Ok(block) => {
                    let id = next_id_templates.fetch_add(1, Ordering::Relaxed);
                    let header = block.header.clone();
                    let pow_state = Arc::new(velkar_pow::State::new(&header));

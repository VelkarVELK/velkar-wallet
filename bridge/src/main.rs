...
use velkar_stratum_bridge::log_colors::LogColors;
use velkar_stratum_bridge::{VelkarApi, StratumServerBridgeConfig as StratumBridgeConfig, listen_and_serve_with_shutdown, prom};
...
fn log_bridge_configuration(config: &BridgeConfig) {
    ...
    tracing::info!("\tvelkard:          {} (shared)", config.global.velkard_address);
...
async fn main() -> Result<(), anyhow::Error> {
    ...
    prom::set_web_status_config(config.global.velkard_address.clone(), config.instances.len());
    ...
    let filter = EnvFilter::try_from_default_env().unwrap_or_else(|_| {
        ...
        if node_mode == NodeMode::Inprocess {
            EnvFilter::new("warn,velkar_stratum_bridge=info,velkar=info,velkard=info,velkard_lib=info,log=info")
        } else {
            EnvFilter::new("warn,velkar_stratum_bridge=info")
        }
    });
    ...
    // are not filtered out by a tracing subscriber installed by velkard.
    let mut inprocess_node: Option<InProcessNode> = None;
    if node_mode == NodeMode::Inprocess {
        let mut node_args: Vec<String> = cli.velkard_args;

        if cli.testnet && !node_args.iter().any(|arg| arg == "--testnet") {
            node_args.push("--testnet".to_string());
        }

        // Add appdir if not provided in velkard_args
        if !node_args.iter().any(|arg| arg.starts_with("--appdir")) {
            let default_appdir = app_dirs::default_inprocess_velkard_appdir();
            ...
            assert!(cli.appdir.is_none(), "appdir should not be specified both in bridge args and velkard args");
        }

        let mut argv: Vec<OsString> = Vec::with_capacity(node_args.len() + 1);
        argv.push(OsString::from("velkard"));
        argv.extend(node_args.iter().map(OsString::from));
        let args = velkard_args::Args::parse(argv).map_err(|e| anyhow::anyhow!("{}", e))?;
    ...
    // Create shared velkar API client (all instances use the same node)
    let velkar_api =
        VelkarApi::new(config.global.velkard_address.clone(), config.global.coinbase_tag_suffix.clone(), shutdown_rx.clone())
            .await
            .map_err(|e| anyhow::anyhow!("Failed to create Velkar API client: {}", e))?;
    ...
    #[cfg(feature = "rkstratum_cpu_miner")]
    {
        if cli.internal_cpu_miner {
            let mining_address = cli
                ...
                .clone()
                .ok_or_else(|| anyhow::anyhow!("--internal-cpu-miner requires --internal-cpu-miner-address <velkar:...>"))?;
            ...
            let cfg = velkar_stratum_bridge::InternalCpuMinerConfig {
                enabled: true,
                mining_address,
                threads,
                throttle,
            ...
            velkar_stratum_bridge::prom::set_internal_cpu_mining_address(cfg.mining_address.clone());

            let metrics = velkar_stratum_bridge::spawn_internal_cpu_miner(Arc::clone(&velkar_api), cfg, shutdown_rx.clone())?;
            velkar_stratum_bridge::set_rkstratum_cpu_miner_metrics(metrics);
            ...
            // This is best-effort and does not affect mining.
            {
                ...
                let internal_metrics = velkar_stratum_bridge::RKSTRATUM_CPU_MINER_METRICS.lock().as_ref().cloned();
                ...
                tokio::spawn(async move {
                    ...
                    loop {
                        ...
                        velkar_stratum_bridge::prom::record_internal_cpu_miner_snapshot(
                            hashes_tried,
                            blocks_submitted,
                            blocks_accepted,
                            hashrate_ghs,
    ...
    for (idx, instance_config) in config.instances.iter().enumerate() {
        ...
        let velkar_api_clone = Arc::clone(&velkar_api);
        ...
        let handle = tokio::spawn(async move {
            ...
            let bridge_config = StratumBridgeConfig {
                ...
                velkard_address: global.velkard_address.clone(),
            ...
            listen_and_serve_with_shutdown(
                bridge_config,
                Arc::clone(&velkar_api_clone),
                if is_first_instance { Some(velkar_api_clone) } else { None },

...
pub struct MiningState {
    ...
    stratum_diff: Arc<Mutex<Option<VelkarDiff>>>,
    max_jobs: u16,
    last_header: Arc<Mutex<Option<velkar_consensus_core::header::Header>>>, // Track previous header for change logging
}
...
impl MiningState {
    ...
    pub fn stratum_diff(&self) -> Option<VelkarDiff> {
        self.stratum_diff.lock().clone()
    }

    /// Set stratum difficulty
    pub fn set_stratum_diff(&self, diff: VelkarDiff) {
    ...
    /// Get last header
    pub fn get_last_header(&self) -> Option<velkar_consensus_core::header::Header> {
        self.last_header.lock().clone()
    }

    /// Set last header
    pub fn set_last_header(&self, header: velkar_consensus_core::header::Header) {

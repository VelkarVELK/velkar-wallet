...
#[cfg(test)]
use velkar_stratum_bridge::BridgeConfig;
...
fn test_config_single_instance_mode() {
    ...
    let yaml = r#"
velkard_address: "127.0.0.1:16110"
stratum_port: ":5555"
min_share_diff: 8192
print_stats: true
"#;
    ...
    assert_eq!(config.global.velkard_address, "127.0.0.1:16110", "Velkard address should be stored in global config");
}
...
fn test_normalize_port_with_colon() {
    ...
    use velkar_stratum_bridge::net_utils::normalize_port;
...
fn test_bind_addr_from_port_with_colon() {
    use velkar_stratum_bridge::net_utils::bind_addr_from_port;
...
fn test_append_line_data_accepts_data_under_limit() {
    use velkar_stratum_bridge::append_line_data;
...
fn test_append_line_data_accepts_incremental_chunks_under_limit() {
    use velkar_stratum_bridge::{MAX_STRATUM_LINE_BYTES, append_line_data};
    let mut buf = String::new();
    let chunk_size = 1024;
...
fn test_total_row_spm_uses_worker_average() {
    use velkar_stratum_bridge::average_worker_spm;
...
fn test_stratum_method_from_str() {
    use velkar_stratum_bridge::jsonrpc_event::StratumMethod;
...
fn test_jsonrpc_event_new() {
    use velkar_stratum_bridge::jsonrpc_event::JsonRpcEvent;
    use serde_json::json;
...
fn test_jsonrpc_event_method_enum() {
    use velkar_stratum_bridge::jsonrpc_event::{JsonRpcEvent, StratumMethod};
...
fn test_jsonrpc_response_new() {
    use velkar_stratum_bridge::jsonrpc_event::{JsonRpcEvent, JsonRpcResponse};
    use serde_json::json;
...
fn test_jsonrpc_response_success() {
    use velkar_stratum_bridge::jsonrpc_event::JsonRpcResponse;
...
fn test_unmarshal_event_basic() {
    use velkar_stratum_bridge::jsonrpc_event::unmarshal_event;
...
fn test_unmarshal_response_success() {
    use velkar_stratum_bridge::jsonrpc_event::unmarshal_response;
...
fn test_error_short_code_display() {
    use velkar_stratum_bridge::errors::ErrorShortCode;
...
fn test_mining_state_initial_values() {
    ...
    // - No stored job IDs
    use velkar_stratum_bridge::mining_state::MiningState;
...
fn test_mining_state_job_management() {
    ...
    use velkar_consensus_core::block::Block;
    use velkar_hashes::Hash;
    use velkar_stratum_bridge::mining_state::{Job, MiningState};
...
// These tests run with: cargo test -p velkar-stratum-bridge --bin stratum-bridge
// Or with CPU miner: cargo test -p velkar-stratum-bridge --features rkstratum_cpu_miner --bin stratum-bridge

#[cfg(test)]
mod integration {
    use velkar_alloc::init_allocator_with_default_settings;
    use velkar_stratum_bridge::{VelkarApi, StratumServerBridgeConfig as StratumBridgeConfig, listen_and_serve_with_shutdown};
    use velkard_lib::args as velkard_args;
    ...
    async fn test_bridge_startup_with_inprocess_node() {
        ...
        let temp_dir = std::env::temp_dir().join(format!("velkard_test_{}", uuid::Uuid::new_v4()));
        ...
        let bridge_handle =
            tokio::spawn(async move { listen_and_serve_with_shutdown::<VelkarApi>(bridge_config, velkar_api, None, shutdown_rx).await });
    ...
    async fn test_bridge_startup_with_cpu_miner_feature() {
        ...
        use velkar_stratum_bridge::InternalCpuMinerConfig;
        let miner_config = InternalCpuMinerConfig {
            enabled: false, // Don't actually mine in test
            mining_address: "velkartest:test".to_string(),
            threads: 1,
            throttle: None,
...
mod comprehensive_tests {
    use velkar_consensus_core::block::Block;
    use velkar_consensus_core::header::Header;
    use velkar_consensus_core::subnets::SubnetworkId;
    use velkar_consensus_core::tx::{ScriptPublicKey, Transaction, TransactionOutput};
    use velkar_hashes::Hash;
    use velkar_stratum_bridge::{
    ...
    /// This helps us test share validation without needing a real Velkar node
    ...
    // - VelkarDiff: Stores difficulty, target, and hash value
    // ========================================================================
    ...
    fn test_difficulty_to_target_calculation() {
        ...
        use velkar_stratum_bridge::hasher::diff_to_target;
    ...
    #[test]
    fn test_velkar_diff_structure() {
        // Test: VelkarDiff structure and methods
        // This demonstrates how difficulty is stored and managed

        let mut diff = VelkarDiff::new();
    ...
    fn test_difficulty_for_different_miners() {
        ...
        let mut diff1 = VelkarDiff::new();
        let mut diff2 = VelkarDiff::new();
        let mut diff3 = VelkarDiff::new();
    ...
    fn test_mining_state_difficulty_storage() {
        ...
        let mut stratum_diff = VelkarDiff::new();
    ...
    fn test_share_handler_worker_stats() {
        ...
        *ctx.wallet_addr.lock() = "velkartest:test".to_string();
    ...
    fn test_worker_prom_session_syncs_start_time_for_current_wallet() {
        ...
        *ctx.worker_name.lock() = "ks0".to_string();
        ...
        let wallet = "velkar:qr8example123456789012345678901234567890123456789012345678901234567890".to_string();
    ...
    // Run with: cargo test -p velkar-stratum-bridge --features rkstratum_cpu_miner
    // ========================================================================
    ...
    fn test_internal_cpu_miner_config_creation() {
        ...
        let config = InternalCpuMinerConfig {
            enabled: true,
            mining_address: "velkartest:test123456789012345678901234567890123456789012345678901234567890".to_string(),
            threads: 4,
        ...
        assert_eq!(config.mining_address, "velkartest:test123456789012345678901234567890123456789012345678901234567890");
    ...
    fn test_internal_miner_metrics_initialization() {
        ...
        use velkar_stratum_bridge::rkstratum_cpu_miner::InternalMinerMetrics;
    ...
    async fn test_internal_cpu_miner_spawn_disabled() {
        ...
        // Create a mock VelkarApi (we won't actually use it when disabled)
        ...
        // without needing a real VelkarApi
    ...
    async fn test_internal_cpu_miner_integration_with_mock_api() {
        ...
        // Note: Full integration requires a real Velkar node (tested in integration tests)
        ...
        // let metrics = spawn_internal_cpu_miner(velkar_api, config, shutdown_rx).await?;
    ...
    fn test_internal_cpu_miner_pow_checking_concept() {
        ...
        use velkar_consensus_core::header::Header;
        use velkar_hashes::Hash;
        use velkar_pow::State as PowState;
    ...
    async fn test_wallet_address_cleaning_with_prefix() {
        // Test: Wallet addresses with velkar:/velkartest:/velkardev: prefixes
        ...
        let event1 = JsonRpcEvent::new(
            Some("1".to_string()),
            "mining.authorize",
            vec![json!("velkar:qr8example123456789012345678901234567890123456789012345678901234567890")],
        );
        ...
        assert!(addr1.starts_with("velkar:"), "Address should have velkar: prefix");
        ...
        // Test velkartest: prefix
        let ctx2 = create_test_context().await;
        let event2 = JsonRpcEvent::new(
            Some("2".to_string()),
            "mining.authorize",
            vec![json!("velkartest:qr8example123456789012345678901234567890123456789012345678901234567890")],
        );
        assert_eq!(event2.params.len(), 1);
        let addr2 = event2.params[0].as_str().unwrap();
        assert!(addr2.starts_with("velkartest:"), "Address should have velkartest: prefix");
        let _result2: Result<(), _> = handle_authorize(ctx2.clone(), event2, None, None).await;

        // Test velkardev: prefix
        let ctx3 = create_test_context().await;
        let event3 = JsonRpcEvent::new(
            Some("3".to_string()),
            "mining.authorize",
            vec![json!("velkardev:qr8example123456789012345678901234567890123456789012345678901234567890")],
        );
        assert_eq!(event3.params.len(), 1);
        let addr3 = event3.params[0].as_str().unwrap();
        assert!(addr3.starts_with("velkardev:"), "Address should have velkardev: prefix");
    ...
    async fn test_wallet_address_cleaning_without_prefix() {
        // Test: Wallet addresses without prefix should get velkar: prefix added
        let ctx = create_test_context().await;
        ...
        assert!(!addr_param.starts_with("velkar:"), "Test address should not have prefix initially");

        // handle_authorize will call clean_wallet which should add velkar: prefix
    ...
    fn test_share_submit_ethereumstratum_format_validation() {
        ...
        // Note: Full submission testing requires a VelkarApi mock, so we test the format here
        ...
        let event = JsonRpcEvent::new(
            ...
            "mining.submit",
            vec![
                json!("velkartest:qr8example123456789012345678901234567890123456789012345678901234567890.worker1"),
                json!("1"),
    ...
    fn test_stratum_context_summary() {
        ...
        *ctx.wallet_addr.lock() = "velkartest:qr8example123456789012345678901234567890123456789012345678901234567890".to_string();
        *ctx.worker_name.lock() = "worker1".to_string();
        *ctx.remote_app.lock() = "BzMiner".to_string();
        ...
        assert_eq!(
            summary.wallet_addr, "velkartest:qr8example123456789012345678901234567890123456789012345678901234567890",

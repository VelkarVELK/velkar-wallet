use crate::{
    ...
    velkarapi::VelkarApi,
    share_handler::{VelkarApiTrait, ShareHandler},
...
/// Start block template listener with concrete VelkarApi
...
pub async fn start_block_template_listener_with_api(
    ...
    let velkar_api_cb = Arc::clone(&velkar_api);

    let block_cb = move || {
        let client_handler = Arc::clone(&client_handler_cb);
        let velkar_api = Arc::clone(&velkar_api_cb);
        tokio::spawn(async move {
            client_handler.new_block_available(velkar_api).await;
        });
    };
...
pub async fn listen_and_serve<T: VelkarApiTrait + Send + Sync + 'static>(
    config: BridgeConfig,
    velkar_api: Arc<T>,
    // Optional: if concrete VelkarApi is provided, use notification-based listener
    concrete_velkar_api: Option<Arc<VelkarApi>>,
) -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
    listen_and_serve_impl(config, velkar_api, concrete_velkar_api, None).await
}

pub async fn listen_and_serve_with_shutdown<T: VelkarApiTrait + Send + Sync + 'static>(
    config: BridgeConfig,
    velkar_api: Arc<T>,
    concrete_velkar_api: Option<Arc<VelkarApi>>,
    shutdown_rx: watch::Receiver<bool>,
) -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
    listen_and_serve_impl(config, velkar_api, concrete_velkar_api, Some(shutdown_rx)).await
}

async fn listen_and_serve_impl<T: VelkarApiTrait + Send + Sync + 'static>(
    config: BridgeConfig,
    velkar_api: Arc<T>,
    concrete_velkar_api: Option<Arc<VelkarApi>>,
    ...
    let authorize_handler = {
        ...
        Arc::new(move |ctx: Arc<StratumContext>, event: JsonRpcEvent| {
            ...
            let event_clone = event.clone();
            Box::pin(async move {
                handle_authorize(ctx_clone, event_clone, Some(client_handler), Some(velkar_api))
                    .await
                    .map_err(|e| Box::new(std::io::Error::other(e.to_string())) as Box<dyn std::error::Error + Send + Sync>)
            })
                as std::pin::Pin<Box<dyn std::future::Future<Output = Result<(), Box<dyn std::error::Error + Send + Sync>>> + Send>>
    ...
    let submit_handler = {
        ...
        Arc::new(move |ctx: Arc<StratumContext>, event: JsonRpcEvent| {
            ...
            Box::pin(async move {
                share_handler
                    .handle_submit(ctx_clone, event, velkar_api)
                    .await
                    .map_err(|e| Box::new(std::io::Error::other(e.to_string())) as Box<dyn std::error::Error + Send + Sync>)
            })
                as std::pin::Pin<Box<dyn std::future::Future<Output = Result<(), Box<dyn std::error::Error + Send + Sync>>> + Send>>
    ...
    // If concrete VelkarApi is provided, use notification-based listener
    // Otherwise, use polling only (fallback for trait objects)
    if let Some(concrete_api) = concrete_velkar_api {
        // We have concrete VelkarApi - use notification-based listener
        let client_handler_cb = Arc::clone(&client_handler);
        let velkar_api_cb = Arc::clone(&velkar_api);
        ...
        // Call the method directly on Arc<VelkarApi> (it's an instance method taking Arc<Self>)
        ...
        // No concrete VelkarApi provided - use polling only
        warn!("Using polling-based block template listener (concrete VelkarApi not provided, notifications not available)");

        let client_handler_poll = Arc::clone(&client_handler);
        let velkar_api_poll = Arc::clone(&velkar_api);
            ...
            loop {
                if let Some(ref mut rx) = shutdown_rx_poll {
                    tokio::select! {
                        ...
                        _ = interval.tick() => {
                            client_handler_poll.new_block_available(Arc::clone(&velkar_api_poll)).await;
                        }
                    }
                } else {
                    interval.tick().await;

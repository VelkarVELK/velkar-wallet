    ...
    N: for<'a> FormatFields<'a> + 'static,
{
    fn format_event(
        ...
        // Special-case forwarded node logs (from `tracing_log::LogTracer`) to match velkard style:
        ...
        if self.apply_colors {
            ...
            if message.contains("[ASIC->BRIDGE]") {
                ...
                // Configuration lines - color the label part (e.g., "\tvelkard:          value")
...
pub(crate) fn init_tracing(
    ...
    // The embedded velkard runtime initializes the global `log` logger via
    // `velkar_core::log::init_logger(...).unwrap()`. If LogTracer grabs the
    // global logger first, velkard panics with SetLoggerError.

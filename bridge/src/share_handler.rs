...
// velkar_pow used inline for PoW validation
...
impl ShareHandler {
    ...
    pub async fn handle_submit(
        ...
        event: JsonRpcEvent,
        velkar_api: Arc<dyn VelkarApiTrait + Send + Sync>,
        ...
        if let Some(Value::String(submitted_identity)) = event.params.first() {
            ...
            let submitted_clean = submitted_address.trim_start_matches("velkar:").trim_start_matches("velkartest:");
            let authorized_clean = wallet_addr.trim_start_matches("velkar:").trim_start_matches("velkartest:");
        ...
        loop {
            ...
            // Use velkar_pow::State for PoW validation against the header's compact bits target.
            ...
            // IMPORTANT: Use velkar_pow's own compact-target handling as the source of truth.
            ...
            debug!("[SUBMIT]   - check_passed(velkar_pow): {}", check_passed);
            ...
            if meets_network_target {
                ...
                // The header comes directly from the Velkar node via get_block_template_call()
                // We preserve: version, bits, timestamp, all hash fields, parents, scores, etc.
                header_clone.nonce = nonce_val;

                // Verify timestamp is still valid (not too old)
                // Velkar typically accepts blocks with timestamps within a reasonable window
                ...
                debug!(
                    ...
                    "Using REAL header from Velkar node block template"
                );
                ...
                debug!("{} {}", LogColors::block("[BLOCK]"), "Calling velkar_api.submit_block()...");

                // Submit block to node
                let block_submit_result = velkar_api.submit_block(block.clone()).await;
                ...
                match block_submit_result {
                    Ok(response) => {
                        ...
                        let velkar_api = Arc::clone(&velkar_api);
                        let block_hash_for_confirm = block_hash.clone();

                        tokio::spawn(async move {
                            for _ in 0..BLOCK_CONFIRM_MAX_ATTEMPTS {
                                match velkar_api.get_current_block_color(&block_hash_for_confirm).await {
                                    Ok(true) => {
...
// Trait for velkar API operations
#[async_trait::async_trait]
pub trait VelkarApiTrait: Send + Sync {
...
mod retention_tests {
    ...
    fn set_client_vardiff_does_not_recreate_pruned_stats() {
        ...
        *ctx.wallet_addr.lock() = "velkartest:ghost".to_string();

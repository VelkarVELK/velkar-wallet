...
function openRowDetailModal(title, rows) {
  ...
  body.innerHTML = (rows || []).map(({ label, value, copyValue }) => {
    ...
    const copy = copyValue != null && String(copyValue) !== ''
      ? `<button type="button" class="bg-surface-2 border border-card px-3 py-2 rounded-lg text-sm font-medium text-white hover:border-velkar-primary shrink-0" data-copy-text="${escapeHtmlAttr(copyValue)}">Copy</button>`
      : '';
    return `
...
function renderWalletSummary(stats, filter) {
  ...
  el.innerHTML = `
    <div class="flex items-start justify-between gap-4">
      ...
      <div class="shrink-0">
        <button type="button" class="bg-surface-1 border border-card px-3 py-1.5 rounded-lg text-xs font-medium text-white hover:border-velkar-primary" data-copy-text="${escapeHtmlAttr(f)}">Copy</button>
      </div>
    </div>
...
async function refresh() {
  ...
  try {
    ...
    document.getElementById('velkardVersion').textContent = status.velkard_version ?? '-';
      ...
      tr.innerHTML = `
        ...
        <td class="py-1.5 pr-3">
          <div class="flex items-center gap-2 min-w-0">
            ...
            ${b.wallet ? `<button type="button" class="bg-surface-1 border border-card px-2 py-0.5 rounded text-xs hover:border-velkar-primary shrink-0" data-copy-text="${escapeHtmlAttr(b.wallet)}">Copy</button>` : ''}
          </div>
        </td>
        ...
        <td class="py-1.5 pr-3">
          <div class="flex items-center gap-2 min-w-0">
            ...
            ${hashFull ? `<button type="button" class="bg-surface-1 border border-card px-2 py-0.5 rounded text-xs hover:border-velkar-primary shrink-0" data-copy-text="${escapeHtmlAttr(hashFull)}">Copy</button>` : ''}
          </div>
        </td>
      `;
      ...
      tr.innerHTML = `
        ...
        <td class="py-1.5 pr-3">
          <div class="flex items-center gap-2 min-w-0">
            ...
            ${wallet ? `<button type="button" class="bg-surface-1 border border-card px-2 py-0.5 rounded text-xs hover:border-velkar-primary shrink-0" data-copy-text="${escapeHtmlAttr(wallet)}">Copy</button>` : ''}
          </div>
        </td>
      ...
      tr.innerHTML = `
        ...
        <td class="py-1.5 pr-3">
          <div class="flex items-center gap-2 min-w-0">
            ...
            ${w.wallet ? `<button type="button" class="bg-surface-1 border border-card px-2 py-0.5 rounded text-xs hover:border-velkar-primary shrink-0" data-copy-text="${String(w.wallet).replace(/&/g,'&amp;').replace(/</g,'&lt;').replace(/>/g,'&gt;').replace(/\"/g,'&quot;').replace(/'/g,'&#39;')}">Copy</button>` : ''}
          </div>
        </td>
    ...
    if (cached) {
      document.getElementById('velkardVersion').textContent = cached.status.velkard_version ?? '-';
      ...
      workers.forEach((w, idx) => {
        ...
        tr.innerHTML = `
          ...
          <td class="py-1.5 pr-3">
            <div class="flex items-center gap-2 min-w-0">
              <span class="min-w-0 truncate" title="${escapeHtmlAttr(w.wallet || '')}">${escapeHtmlAttr(w.wallet || '-')}</span>
              ${w.wallet ? `<button type="button" class="bg-surface-1 border border-card px-2 py-0.5 rounded text-xs hover:border-velkar-primary shrink-0" data-copy-text="${escapeHtmlAttr(w.wallet)}">Copy</button>` : ''}
            </div>
          </td>
          <td class="py-1.5 pr-3">${formatHashrateHs((w.hashrate || 0) * 1e9)}</td>
          <td class="py-1.5 pr-3">${w.currentDifficulty != null ? formatDifficulty(w.currentDifficulty) : '-'}</td>

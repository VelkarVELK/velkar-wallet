...
use velkar_core::velkard_env::version;
use velkar_notify::address::tracker::Tracker;
use velkar_utils::networking::ContextualNetAddress;
use velkar_wrpc_server::address::WrpcNetAddress;
...
#[cfg(feature = "devnet-prealloc")]
use velkar_consensus_core::tx::{TransactionOutpoint, UtxoEntry};
#[cfg(feature = "devnet-prealloc")]
use velkar_txscript::pay_to_address_script;
#[cfg(feature = "devnet-prealloc")]
use std::sync::Arc;
...
impl Args {
    ...
    #[cfg(feature = "devnet-prealloc")]
    pub fn generate_prealloc_utxos(&self, num_prealloc_utxos: u64) -> velkar_consensus_core::utxo::utxo_collection::UtxoCollection {
    ...
    pub fn network(&self) -> NetworkId {
        match (self.velkarnet, self.testnet, self.devnet, self.simnet) {
...
pub fn cli() -> Command {
    ...
    let cmd = Command::new("velkard")
        .about(format!("{} (Velkar) v{}", env!("CARGO_PKG_DESCRIPTION"), version()))
        .version(env!("CARGO_PKG_VERSION"))
        .arg(arg!(-C --configfile <CONFIG_FILE> "Path of config file.").env("VELKARD_CONFIGFILE"))
        .arg(arg!(-b --appdir <DATA_DIR> "Directory to store data.").env("VELKARD_APPDIR"))
        .arg(arg!(--logdir <LOG_DIR> "Directory to log output.").env("VELKARD_LOGDIR"))
        .arg(arg!(--nologfiles "Disable logging to files.").env("VELKARD_NOLOGFILES"))
        .arg(
            Arg::new("async_threads")
                .short('t')
                .long("async-threads")
                .env("VELKARD_ASYNC_THREADS")
        ...
        .arg(arg!(--unsaferpc "Enable RPC commands which affect the state of the node").env("VELKARD_UNSAFERPC"))
        .arg(
            Arg::new("connect-peers")
                .long("connect")
                .env("VELKARD_CONNECTPEERS")
                .value_name("IP[:PORT]")
        ...
        .arg(
            Arg::new("rpcmaxclients")
                .long("rpcmaxclients")
                .env("VELKARD_RPCMAXCLIENTS")
                .value_name("rpcmaxclients")
                .require_equals(true)
                .value_parser(clap::value_parser!(usize))
                .help("Max number of RPC clients for standard connections (default: 128)."),
        )
        .arg(arg!(--"reset-db" "Reset database before starting node. It's needed when switching between subnetworks.").env("VELKARD_RESET_DB"))
        .arg(arg!(--"enable-unsynced-mining" "Allow the node to accept blocks from RPC while not synced (this flag is mainly used for testing)").env("VELKARD_ENABLE_UNSYNCED_MINING"))
        .arg(
            Arg::new("enable-mainnet-mining")
                .long("enable-mainnet-mining")
                .env("VELKARD_ENABLE_MAINNET_MINING")
                .action(ArgAction::SetTrue)
                .hide(true)
                .help("Allow mainnet mining (currently enabled by default while the flag is kept for backwards compatibility)"),
        )
        .arg(arg!(--utxoindex "Enable the UTXO index").env("VELKARD_UTXOINDEX"))
        .arg(
            Arg::new("max-tracked-addresses")
                .long("max-tracked-addresses")
                .env("VELKARD_MAX_TRACKED_ADDRESSES")
                .require_equals(true)
                .value_parser(clap::value_parser!(usize))
...
0, Tracker::MAX_ADDRESS_UPPER_BOUND, Tracker::DEFAULT_MAX_ADDRESSES)),
        ...
        .arg(arg!(--velkarnet "Use the Velkar main network (default if no network flag is set)").env("VELKARD_VELKARNET"))
        .arg(arg!(--testnet "Use the test network").env("VELKARD_TESTNET"))
        .arg(
            Arg::new("netsuffix")
                .long("netsuffix")
                .env("VELKARD_NETSUFFIX")
                .value_name("netsuffix")
                .require_equals(true)
                .value_parser(clap::value_parser!(u32))
                .help("Testnet network suffix number"),
        )
        .arg(arg!(--devnet "Use the development test network").env("VELKARD_DEVNET"))
        .arg(arg!(--simnet "Use the simulation test network").env("VELKARD_SIMNET"))
        .arg(arg!(--archival "Run as an archival node: avoids deleting old block data when moving the pruning point (Warning: heavy disk usage)").env("VELKARD_ARCHIVAL"))
        .arg(arg!(--sanity "Enable various sanity checks which might be compute-intensive (mostly performed during pruning)").env("VELKARD_SANITY"))
        .arg(arg!(--yes "Answer yes to all interactive console questions").env("VELKARD_NONINTERACTIVE"))
        .arg(
            Arg::new("user_agent_comments")
                .long("uacomment")
                .env("VELKARD_USER_AGENT_COMMENTS")
                .action(ArgAction::Append)
                .require_equals(true)
        ...
        .arg(arg!(--"perf-metrics" "Enable performance metrics: cpu, memory, disk io usage").env("VELKARD_PERF_METRICS"))
        .arg(
            Arg::new("perf-metrics-interval-sec")
                .long("perf-metrics-interval-sec")
                .env("VELKARD_PERF_METRICS_INTERVAL_SEC")
                .require_equals(true)
                .value_parser(clap::value_parser!(u64))
                .help("Interval in seconds for performance metrics collection."),
        )
        .arg(arg!(--"disable-upnp" "Disable upnp").env("VELKARD_DISABLE_UPNP"))
        .arg(arg!(--"nodnsseed" "Disable DNS seeding for peers").env("VELKARD_NODNSSEED"))
        .arg(arg!(--"nogrpc" "Disable gRPC server").env("VELKARD_NOGRPC"))
        .arg(
            Arg::new("ram-scale")
                .long("ram-scale")
                .env("VELKARD_RAM_SCALE")
                .require_equals(true)
...
impl Args {
        ...
        T: Into<OsString> + Clone,
    {
        ...
        let args = Args {
            ...
            velkarnet: arg_match_unwrap_or::<bool>(&m, "velkarnet", defaults.velkarnet),
...
/*
  ...
  -a, --addpeer=                            Add a peer to connect with at startup
      ...
      --rpccert=                            File containing the certificate file (default:
                                            /Users/aspect/Library/Application Support/Velkar/rpc.cert)
      --rpckey=                             File containing the certificate key (default:
                                            /Users/aspect/Library/Application Support/Velkar/rpc.key)
  ...
  -d, --loglevel=                           Logging level for all subsystems {trace, debug, info, warn, error,
      ...
      --minrelaytxfee=                      The minimum transaction fee in VELK/kB to be considered a non-zero fee.
                                            (default: 1e-05)

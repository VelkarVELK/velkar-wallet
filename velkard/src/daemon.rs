...
use velkar_consensus_notify::{root::ConsensusNotificationRoot, service::NotifyService};
use velkar_core::{core::Core, debug, info};
use velkar_core::{velkard_env::{name, version}, task::tick::TickService};
use velkar_database::{
    prelude::{CachePolicy, DbWriter, DirectDbWriter, RocksDbPreset},
    registry::DatabaseStorePrefixes,
};
use velkar_grpc_server::service::GrpcService;
use velkar_notify::{address::tracker::Tracker, subscription::context::SubscriptionContext};
use velkar_p2p_lib::Hub;
use velkar_p2p_mining::rule_engine::MiningRuleEngine;
use velkar_rpc_service::service::RpcCoreService;
use velkar_txscript::caches::TxScriptCacheCounters;
use velkar_utils::git;
use velkar_utils::networking::ContextualNetAddress;
use velkar_utils::sysinfo::SystemInfo;
use velkar_utils_tower::counters::TowerConnectionCounters;

use velkar_addressmanager::AddressManager;
use velkar_consensus::{
...
use velkar_p2p_flows::{flow_context::FlowContext, service::P2pService};

use velkar_perf_monitor::{builder::Builder as PerfMonitorBuilder, counters::CountersSnapshot};
use velkar_utxoindex::{UtxoIndex, api::UtxoIndexProxy};
use velkar_wrpc_server::service::{Options as WrpcServerOptions, WebSocketCounters as WrpcServerCounters, WrpcEncoding, WrpcService};

/// Desired soft FD limit that needs to be configured
/// for the Velkar node process.
pub const DESIRED_DAEMON_SOFT_FD_LIMIT: u64 = 8 * 1024;
/// Minimum acceptable soft FD limit for the Velkar
/// process. (Velkar will operate with the minimal
...
pub fn get_app_dir() -> PathBuf {
    #[cfg(target_os = "windows")]
    return get_home_dir().join("velkar");
    #[cfg(not(target_os = "windows"))]
    return get_home_dir().join(".velkar");
}
...
fn request_database_deletion_approval(approve: bool) -> bool {
    let msg = "Node database is from a different Velkard *DB* version and needs to be fully deleted, do you confirm the delete? (y/n)";
    get_user_approval_or_exit(msg, approve);
    info!("Deleting databases from previous Velkard version");
...
/// the application folder that contains Velkar logs and the database.
    ...
    pub fn from_args(args: &Args) -> Self {
        ...
        cfg_if::cfg_if! {
            if #[cfg(feature = "semaphore-trace")] {
                velkar_core::log::init_logger(log_dir.as_deref(), &format!("{},{}=debug", args.log_level, velkar_utils::sync::semaphore_module_path()));
            } else {
                velkar_core::log::init_logger(log_dir.as_deref(), &args.log_level);
            }
        };
...
pub fn create_core_with_runtime(runtime: &Runtime, args: &Args, fd_total_budget: i32) -> (Arc<Core>, Arc<RpcCoreService>) {
    ...
    info!("{} v{}", name(), git::with_short_hash(version()));
...
do you confirm? (answer y/n or pass --yes to the Velkar command line to confirm all interactive questions)";
    ...
    let mut meta_db = velkar_database::prelude::ConnBuilder::default()
    ...
    if !is_db_reset_needed && (args.testnet || args.devnet || args.simnet) {
        ...
        match active_consensus_dir_name {
            Some(dir_name) => {
                let consensus_db = velkar_database::prelude::ConnBuilder::default()
    ...
    // Reset Condition: Need to reset if we're upgrading from an older node DB version
...
Do you confirm? (y/n)";
    ...
    if is_db_reset_needed && !args.reset_db {
        ...
        // Reopen the DB
        meta_db = velkar_database::prelude::ConnBuilder::default()
    ...
    let perf_monitor = if args.perf_metrics {
        let cb = move |counters: CountersSnapshot| {
            debug!("[{}] {}", velkar_perf_monitor::SERVICE_NAME, counters.to_process_metrics_display());
            debug!("[{}] {}", velkar_perf_monitor::SERVICE_NAME, counters.to_io_metrics_display());
            #[cfg(feature = "heap")]
            debug!("[{}] heap stats: {:?}", velkar_perf_monitor::SERVICE_NAME, dhat::HeapStats::get());
        };
    ...
    let index_service: Option<Arc<IndexService>> = if args.utxoindex {
        ...
        let utxoindex_db = velkar_database::prelude::ConnBuilder::default()

extern crate velkar_consensus;
extern crate velkar_core;
extern crate velkar_hashes;

use std::sync::Arc;

use velkar_alloc::init_allocator_with_default_settings;
use velkar_core::{info, signals::Signals};
...
pub fn main() {
    #[cfg(feature = "heap")]
    let _profiler = dhat::Profiler::builder().file_name("velkard-heap.json").build();
    ...
    match fd_budget::try_set_fd_limit(DESIRED_DAEMON_SOFT_FD_LIMIT) {
        Ok(limit) => {
            if limit < MINIMUM_DAEMON_SOFT_FD_LIMIT {
                ...
                println!("The velkard node requires a setting of at least {DESIRED_DAEMON_SOFT_FD_LIMIT} to operate properly.");

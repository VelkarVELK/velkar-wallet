...
use velkar_consensus_core::{BlockHashSet, tx::ScriptPublicKeys, utxo::utxo_diff::UtxoDiff};
use velkar_consensusmanager::{ConsensusManager, ConsensusResetHandler};
use velkar_core::{info, trace};
use velkar_database::prelude::{DB, StoreError, StoreResult};
...
const RESYNC_CHUNK_SIZE: usize = 2048; // Increased from 1k (used in go-velkard), for quicker resets, while still having a low memory footprint.

/// UtxoIndex indexes `CompactUtxoEntryCollections` by [`ScriptPublicKey`](velkar_consensus_core::tx::ScriptPublicKey),
...
impl UtxoIndexApi for UtxoIndex {
    ...
    fn get_all_outpoints(&self) -> StoreResult<std::collections::HashSet<velkar_consensus_core::tx::TransactionOutpoint>> {
